...
class ECoGDataset(Dataset):
    ...
    def zscore(self, ecog, badelec, axis=None):
        if badelec.shape[0] != 0:
            statics_ecog = (
                np.delete(ecog, badelec, axis=1).mean(axis=axis, keepdims=True) + 1e-10,
                np.delete(ecog, badelec, axis=1).std(axis=axis, keepdims=True) + 1e-10,
            )
        else:
            statics_ecog = (
                ecog.mean(axis=axis, keepdims=True) + 1e-10,
                ecog.std(axis=axis, keepdims=True) + 1e-10,
            )
    ...
    def rearrange(self, data, crop=None, mode="ecog"):
        ...
        if mode == "ecog":
            ...
            for i in range(len(rows)):
                data_new[:, rows[i], starts[i] : ends[i] : strides[i]] = data[
            ...
            x_new, y_new = np.meshgrid(
                np.arange(15).astype(np.float32), np.arange(15).astype(np.float32)
            )
            ...
            if crop is None:
                ...
                return np.reshape(
                    data_new[crop[0] : crop[0] + crop[2], crop[1] : crop[1] + crop[3]],
                    [-1, data.shape[-1]],
                )  # Nx3
            ...
            for i in range(len(rows)):
                region_new[rows[i], starts[i] : ends[i] : strides[i]] = data[
                    electrodes[i] : electrodes[i + 1]
                ]
            if crop is None:
    ...
    def rearrange_LD(self, data, crop=None, mode="ecog", nums=64):
        ...
        if mode == "ecog":
            ...
            x_new, y_new = np.meshgrid(
                np.arange(square_shape).astype(np.float32),
                np.arange(square_shape).astype(np.float32),
            )
            x_org = x_new.flatten()
            y_org = y_new.flatten()
    ...
    def transform(
        ...
        return augmentor.Compose(
            augmentor.RandomizedDropout(0.2, apply_p=0.2),
        ...
        if self.ReshapeAsGrid:
            if select:
                ...
                for region in select:
                    region_ind = [
                        region.encode() == regions[i] for i in range(regions.shape[0])
                    ]
            ...
            return (
                ...
                regions[region_ind],
                mask[region_ind],
                mni_coord[region_ind],
                mni_coord_raw[region_ind],
                T1_coord[region_ind],
                T1_coord_raw[region_ind],
            )
    ...
    def __init__(
        ...
        self.DEBUG = DEBUG
        self.FAKE_LD = (
            FAKE_LD  # only true for HB e2a exp but only use first 64 electrodes!
        )
        ...
        self.SelectRegion = []
        [
            self.SelectRegion.extend(self.cortex[area])
            for area in cfg.DATASET.SELECTREGION
        ]
        self.BlockRegion = []
        ...
        if ReshapeAsGrid is not None:
            ...
            self.ReshapeAsGrid = (
                False
                if ("lstm_" in cfg.MODEL.MAPPING_FROM_ECOG)
                or ("Transformer" in cfg.MODEL.MAPPING_FROM_ECOG)
                or ("Performer" in cfg.MODEL.MAPPING_FROM_ECOG)
                or (
                    "ECoGMappingPerformer2Dconv_downsample1_posemb"
                    in cfg.MODEL.MAPPING_FROM_ECOG
                )
                else True
            )
        ...
        print(
            "cfg.MODEL.NOISE_DB,max_db=cfg.MODEL.MAX_DB",
            cfg.MODEL.NOISE_DB,
            cfg.MODEL.MAX_DB,
        )
        ...
        for subj in self.ReqSubjDict:
            ...
            if cfg.DATASET.PROD:
                if self.FAKE_REPROD:
                    self.TestNum_cum = np.append(
                        self.TestNum_cum,
                        np.array(train_param["Subj"][subj]["TestNum_imagine"])
                        .sum()
                        .astype(np.int32),
                    )
                    ...
                    self.TestNum_cum = np.append(
                        self.TestNum_cum,
                        np.array(train_param["Subj"][subj]["TestNum"])
                        .sum()
                        .astype(np.int32),
                    )
                ...
                self.TestNum_cum = np.append(
                    self.TestNum_cum,
                    np.array(train_param["Subj"][subj]["TestNum_percept"])
                    .sum()
                    .astype(np.int32),
                )
            ...
            for xx, task_to_use in enumerate(tasks):
                ...
                if self.low_density and not self.FAKE_LD:
                    analysispath_task = "/home/xc1490/home/projects/ecog/ALAE_1023/data/data/refined_events/{}/{}/".format(
                        subj, task_to_use
                    )
                ...
                event_range = (
                    None
                    if "EventRange" not in subj_param.keys()
                    else subj_param["EventRange"]
                )
                # bad_samples = [] if "BadSamples" not in subj_param.keys() else subj_param["BadSamples"]
                #
                if self.low_density and not self.FAKE_LD:
                    analysis_event = pd.read_hdf(
                        os.path.join(analysispath_task, "Events.h5"), "/events"
                    )
                else:
                    try:
                        analysis_event = scipy.io.loadmat(
                            os.path.join(analysispath_task, "Events.mat")
                        )["Events"]
                    except:
                        ...
                        # analysis_event = fileh5['Events']
                        analysis_event = mat73.loadmat(
                            os.path.join(analysispath_task, "Events.mat")
                        )["Events"]
                        analysis_event = {
                            key: [np.array([[[val]] for val in value])]
                            for (key, value) in analysis_event.items()
                        }  # this wrap is used to accommodate the previous codes
                ...
                if self.low_density and not self.FAKE_LD:
                    ...
                    start_ind_wave = np.array(
                        [
                            start_ind_wave[i][0, 0]
                            for i in range(start_ind_wave.shape[0])
                        ],
                        dtype=np.float,
                    )[:event_range]
                    end_ind_wave = analysis_event["offset"][0]
                    end_ind_wave = np.array(
                        [end_ind_wave[i][0, 0] for i in range(end_ind_wave.shape[0])],
                        dtype=np.float,
                    )[:event_range]
                if self.Prod:
                    if self.FAKE_REPROD:
                        start_ind_re_wave = (
                            end_ind_wave + 128
                        )  # 0.25s after stim offset
                        end_ind_re_wave = (
                            start_ind_re_wave + 256
                        )  # 0.50s after start_ind_re_wave
                    else:
                        if self.low_density and not self.FAKE_LD:
                            ...
                            start_ind_re_wave = np.nan_to_num(
                                np.array(
                                    [
                                        start_ind_re_wave[i][0, 0]
                                        for i in range(start_ind_re_wave.shape[0])
                                    ],
                                    dtype=np.float,
                                )[:event_range]
                            )
                            ...
                            end_ind_re_wave = np.nan_to_num(
                                np.array(
                                    [
                                        end_ind_re_wave[i][0, 0]
                                        for i in range(end_ind_re_wave.shape[0])
                                    ],
                                    dtype=np.float,
                                )[:event_range]
                            )
                ...
                if HD:
                    start_ind = (
                        start_ind_wave * 1.0 / self.ORG_WAVE_FS * self.DOWN_ECOG_FS
                    ).astype(
                        np.int64
                    )  # in ECoG sample
                    start_ind_wave_down = (
                        start_ind_wave * 1.0 / self.ORG_WAVE_FS * self.DOWN_WAVE_FS
                    ).astype(np.int64)
                    end_ind = (
                        end_ind_wave * 1.0 / self.ORG_WAVE_FS * self.DOWN_ECOG_FS
                    ).astype(
                        np.int64
                    )  # in ECoG sample
                    end_ind_wave_down = (
                        end_ind_wave * 1.0 / self.ORG_WAVE_FS * self.DOWN_WAVE_FS
                    ).astype(np.int64)
                    start_ind_valid = np.delete(start_ind, bad_samples)
                    end_ind_valid = np.delete(end_ind, bad_samples)
                    start_ind_wave_down_valid = np.delete(
                        start_ind_wave_down, bad_samples
                    )
                    ...
                    start_ind = (
                        start_ind_wave * 1.0 / self.ORG_ECOG_FS_NY * self.DOWN_ECOG_FS
                    ).astype(np.int64)
                    start_ind_wave_down = (
                        start_ind_wave * 1.0 / self.DOWN_ECOG_FS * self.DOWN_WAVE_FS
                    ).astype(np.int64)
                    end_ind = (
                        end_ind_wave * 1.0 / self.ORG_ECOG_FS_NY * self.DOWN_ECOG_FS
                    ).astype(np.int64)
                    end_ind_wave_down = (
                        end_ind_wave * 1.0 / self.DOWN_ECOG_FS * self.DOWN_WAVE_FS
                    ).astype(np.int64)
                    if self.low_density and not self.FAKE_LD:
                        ...
                        if self.Prod:
                            ...
                            bad_samples_HD = analysis_event["badevent"][0]
                        bad_samples_HD = np.asarray(
                            [
                                bad_samples_HD[i][0, 0]
                                for i in range(bad_samples_HD.shape[0])
                            ]
                        )
                    ...
                    if subj == "NY798":
                        ...
                        bad_samples_HD = np.where(
                            np.logical_or(
                                np.logical_or(bad_samples_HD == 1, bad_samples_HD == 2),
                                bad_samples_HD == 4,
                            )
                        )[0]
                    ...
                    start_ind_wave_down_valid = np.delete(
                        start_ind_wave_down, bad_samples_HD
                    )
                    end_ind_wave_down_valid = np.delete(
                        end_ind_wave_down, bad_samples_HD
                    )
                    if self.Prod:
                        start_ind_re = (
                            start_ind_re_wave
                        ...
                        start_ind_re_wave_down_valid = np.delete(
                            start_ind_re_wave_down, bad_samples_HD
                        )
                        end_ind_re_wave_down_valid = np.delete(
                            end_ind_re_wave_down, bad_samples_HD
                        )

                baseline_ind = np.concatenate(
                    [
                        np.arange(
                            start_ind_valid[i] - self.DOWN_ECOG_FS // 4,
                            start_ind_valid[i] - self.DOWN_ECOG_FS // 20,
                        )
                        for i in range(len(start_ind_valid))
                    ]
                )  # baseline: 1/4 s - 1/20 s before stimulis onset
                baseline_ind_spec = np.concatenate(
                    [
                        np.arange(
                            (
                                start_ind_valid[i]
                ...
                )  # baseline: 1/4 s - 1/8 s before stimulis onset

                # if self.process_ecog:
                ecog = (
                    signal.resample_poly(
                        ecog, self.DOWN_ECOG_FS * 10000, 30517625, axis=0
                    )
                    if HD
                    else signal.resample_poly(
                        ecog, self.DOWN_ECOG_FS, self.ORG_ECOG_FS_NY, axis=0
                    )
                )  # resample to 125 hz
                # import pdb; pdb.set_trace()
                ...
                # import pdb; pdb.set_trace()
                baseline_ind_spec_perceptnoise = np.concatenate(
                    [
                        np.arange(
                            (
                                start_ind_valid[i]
                ...
                statics_ecog = (
                    baseline.mean(axis=0, keepdims=True) + 1e-10,
                    np.sqrt(baseline.var(axis=0, keepdims=True)) + 1e-10,
                )
                ...
                end_ind_valid_train_ += [
                    end_ind_valid[:TestInd] + np.cumsum(ecog_len_)[-2]
                ]
                start_ind_valid_train = (
                    start_ind_valid[:TestInd] + np.cumsum(ecog_len_)[-2]
                )
                start_ind_valid_train_ += [start_ind_valid_train]
                start_ind_wave_down_train = start_ind_wave_down[:TestInd] + (
                    np.cumsum(ecog_len_)[-2]
                    * 1.0
                    / self.DOWN_ECOG_FS
                    * self.DOWN_WAVE_FS
                ).astype(np.int64)
                start_ind_wave_down_train_ += [start_ind_wave_down_train]
                end_ind_wave_down_train = end_ind_wave_down[:TestInd] + (
                    np.cumsum(ecog_len_)[-2]
                    * 1.0
                    / self.DOWN_ECOG_FS
                    * self.DOWN_WAVE_FS
                ...
                start_ind_wave_down_valid_train = start_ind_wave_down_valid[
                    :TestInd
                ] + (
                    np.cumsum(ecog_len_)[-2]
                    * 1.0
                ...
                end_ind_wave_down_valid_train = end_ind_wave_down_valid[:TestInd] + (
                    np.cumsum(ecog_len_)[-2]
                    * 1.0
                    / self.DOWN_ECOG_FS
                    * self.DOWN_WAVE_FS
                ...
                start_ind_valid_test = (
                    start_ind_valid[TestInd:] + np.cumsum(ecog_len_)[-2]
                )
                start_ind_valid_test_ += [start_ind_valid_test]
                start_ind_wave_down_test = start_ind_wave_down[TestInd:] + (
                    np.cumsum(ecog_len_)[-2]
                    * 1.0
                    / self.DOWN_ECOG_FS
                    * self.DOWN_WAVE_FS
                ...
                end_ind_wave_down_test = end_ind_wave_down[TestInd:] + (
                    np.cumsum(ecog_len_)[-2]
                    * 1.0
                    / self.DOWN_ECOG_FS
                    * self.DOWN_WAVE_FS
                ...
                start_ind_wave_down_valid_test = start_ind_wave_down_valid[TestInd:] + (
                    np.cumsum(ecog_len_)[-2]
                    * 1.0
                    / self.DOWN_ECOG_FS
                    * self.DOWN_WAVE_FS
                ...
                end_ind_wave_down_valid_test = end_ind_wave_down_valid[TestInd:] + (
                    np.cumsum(ecog_len_)[-2]
                    * 1.0
                    / self.DOWN_ECOG_FS
                    * self.DOWN_WAVE_FS
                ...
                if self.Prod:
                    start_ind_re_train_ += [
                        start_ind_re[:TestInd] + np.cumsum(ecog_len_)[-2]
                    ]
                    end_ind_re_train_ += [
                        end_ind_re[:TestInd] + np.cumsum(ecog_len_)[-2]
                    ]
                    end_ind_re_valid_train_ += [
                        end_ind_re_valid[:TestInd] + np.cumsum(ecog_len_)[-2]
                    ]
                    start_ind_re_validtrain_ = (
                        start_ind_re_valid[:TestInd] + np.cumsum(ecog_len_)[-2]
                    )
                    start_ind_re_valid_train_ += [start_ind_re_validtrain_]
                    start_ind_re_wave_downtrain_ = start_ind_re_wave_down[:TestInd] + (
                        np.cumsum(ecog_len_)[-2]
                        * 1.0
                        / self.DOWN_ECOG_FS
                        * self.DOWN_WAVE_FS
                    ...
                    end_ind_re_wave_downtrain_ = end_ind_re_wave_down[:TestInd] + (
                        np.cumsum(ecog_len_)[-2]
                        * 1.0
                        / self.DOWN_ECOG_FS
                        * self.DOWN_WAVE_FS
                    ...
                    start_ind_re_wave_down_validtrain_ = start_ind_re_wave_down_valid[
                        :TestInd
                    ] + (
                        np.cumsum(ecog_len_)[-2]
                        * 1.0
                    ...
                    start_ind_re_wave_down_valid_train_ += [
                        start_ind_re_wave_down_validtrain_
                    ]
                    end_ind_re_wave_down_validtrain_ = end_ind_re_wave_down_valid[
                        :TestInd
                    ] + (
                        np.cumsum(ecog_len_)[-2]
                        * 1.0
                    ...
                    end_ind_re_wave_down_valid_train_ += [
                        end_ind_re_wave_down_validtrain_
                    ]
                    ...
                    start_ind_re_validtest_ = (
                        start_ind_re_valid[TestInd:] + np.cumsum(ecog_len_)[-2]
                    )
                    start_ind_re_valid_test_ += [start_ind_re_validtest_]
                    start_ind_re_wave_downtest_ = start_ind_re_wave_down[TestInd:] + (
                        np.cumsum(ecog_len_)[-2]
                        * 1.0
                        / self.DOWN_ECOG_FS
                        * self.DOWN_WAVE_FS
                    ...
                    end_ind_re_wave_downtest_ = end_ind_re_wave_down[TestInd:] + (
                        np.cumsum(ecog_len_)[-2]
                        * 1.0
                        / self.DOWN_ECOG_FS
                        * self.DOWN_WAVE_FS
                    ...
                    start_ind_re_wave_down_validtest_ = start_ind_re_wave_down_valid[
                        TestInd:
                    ] + (
                        np.cumsum(ecog_len_)[-2]
                        * 1.0
                    ...
                    start_ind_re_wave_down_valid_test_ += [
                        start_ind_re_wave_down_validtest_
                    ]
                    end_ind_re_wave_down_validtest_ = end_ind_re_wave_down_valid[
                        TestInd:
                    ] + (
                        np.cumsum(ecog_len_)[-2]
                        * 1.0
                    ...
                    end_ind_re_wave_down_valid_test_ += [
                        end_ind_re_wave_down_validtest_
                    ]
                if not self.Prod:
                    ...
                    spkr = np.zeros(
                        [end_ind[-1], self.SpecBands]
                    )  # np.zeros(ecog.shape[0],32)
                    if self.phoneme:
                        phoneme = np.load(
                            "/scratch/akg404/SharedECoGData/FunctionalMapping/manually_refine_onsetlabel/manually_refined_perception_new/manually_refined_phoneme_indicator_"
                            + subj
                            + "_"
                            + task_to_use
                            + "_stim.npy"
                ...
                if xx == 0:
                    statics_spkr = (
                        samples_for_statics.mean(axis=0, keepdims=True) + 1e-10,
                        np.sqrt(samples_for_statics.var(axis=0, keepdims=True)) + 1e-10,
                    )
                # print(statics_spkr)
                if self.Wipenoise:
                ...
                spkr = (np.clip(spkr, 0.0, 70.0) - 35.0) / 35.0
                # spkr = (spkr - statics_spkr[0])/statics_spkr[1]
                spkr_trim = np.zeros(
                    [
                        int(ecog.shape[0] * 1.0 / self.DOWN_ECOG_FS * self.DOWN_TF_FS),
                        spkr.shape[1],
                    ]
                )
                ...
                if not self.Prod:
                    wavedata = h5py.File(
                        os.path.join(datapath_task, "spkr_16k.mat"), "r"
                    )
                ...
                if cfg.MODEL.WAVE_BASED:
                    wave_spec = (
                        wave2spec(
                            torch.tensor(wavearray.T),
                            n_fft=N_FFT_num,
                            noise_db=cfg.MODEL.NOISE_DB,
                            max_db=cfg.MODEL.MAX_DB,
                            power=2 if cfg.MODEL.POWER_SYNTH else 1,
                        )[0]
                        .detach()
                        .cpu()
                        .numpy()
                    ...
                    baseline_ind_spec = baseline_ind_spec[
                        baseline_ind_spec < wave_spec.shape[0]
                    ]
                    ...
                    for samples in range(start_ind.shape[0]):
                        if not np.isnan(start_ind[samples]):
                            if samples == 0:
                                ...
                                wave_spec[
                                    end_ind[samples - 1] : start_ind[samples]
                                ] = -1
                            if samples == start_ind.shape[0] - 1:
                ...
                if self.Prod:
                    if self.FAKE_REPROD:
                        ...
                        try:
                            spkr_redata = h5py.File(
                                os.path.join(
                                    datapath_task,
                                    "TFzoom" + str(self.SpecBands) + "_16k_log10.mat",
                                ),
                                "r",
                            )
                        except:
                            spkr_redata = h5py.File(
                                os.path.join(
                                    datapath_task,
                                    "TFzoom" + str(self.SpecBands) + "_16k.mat",
                                ),
                                "r",
                            )
                        spkr_re = np.asarray(spkr_redata["TFlog"])
                        spkr_re = signal.resample(
                            spkr_re,
                            int(
                                1.0
                                * spkr_re.shape[0]
                    ...
                    if self.phoneme:
                        if self.FAKE_REPROD:
                            ...
                            phoneme_re = np.load(
                                os.path.join(
                                    datapath_task,
                                    "manually_refined_phoneme_indicator_"
                                    + subj
                                    + "_"
                                    + task_to_use
                                    + ".npy",
                    ...
                    if self.formant:
                        if self.FAKE_REPROD:
                            ...
                            formant_re = np.load(
                                "/scratch/xc1490/projects/ecog/ALAE_1023/data/data/formant_label/formant_label_praat_{}_{}.npy".format(
                                    subj, task_to_use
                                )
                            )
                    if self.pitch:
                        if self.FAKE_REPROD:
                            ...
                            pitch_re = np.load(
                                "/scratch/xc1490/projects/ecog/ALAE_1023/data/data/pitch_label/pitch_label_praat_{}_{}.npy".format(
                                    subj, task_to_use
                                )
                            )
                    if self.intensity:
                        if self.FAKE_REPROD:
                            ...
                            intensity_re = np.load(
                                "/scratch/xc1490/projects/ecog/ALAE_1023/data/data/intensity_label/intensity_label_praat_{}_{}.npy".format(
                                    subj, task_to_use
                                )
                            )
                    ...
                    if not HD:
                        samples_for_statics_re = spkr_re[
                            start_ind_re[0] : start_ind_re[-1]
                        ]
                    # samples_for_statics_re = spkr_re[statics_samples_spkr_re[ds][file*2]:statics_samples_spkr_re[ds][file*2+1]]
                    if xx == 0:
                        statics_spkr_re = (
                            samples_for_statics_re.mean(axis=0, keepdims=True) + 1e-10,
                            np.sqrt(samples_for_statics_re.var(axis=0, keepdims=True))
                            + 1e-10,
                        )
                    # print(statics_spkr_re)
                    if self.Wipenoise:
                        if subj is not "NY717" or (
                            task_to_use is not "VisRead" and task_to_use is not "PicN"
                        ):
                    ...
                    if self.intensity:
                        ...
                        if intensity_re.shape[0] > intensity_re_trim.shape[0]:
                            intensity_re_trim = intensity_re[
                                : intensity_re_trim.shape[0]
                            ]
                    ...
                    if not self.FAKE_REPROD:
                        if self.use_denoise:
                            ...
                            wave_redata = h5py.File(
                                os.path.join(datapath_task, "zoom_16k_denoise_nr.mat"),
                                "r",
                            )
                            wave_rearray = wave_redata["zoom"][:].reshape(
                                -1, 1
                            )  # NY668 has two waveform channels
                            # wave_rearray = wave_rearray.T
                            wave_redata_denoise = h5py.File(
                                os.path.join(datapath_task, "zoom_16k_denoise_nr.mat"),
                                "r",
                            )  # h5py.File(os.path.join(datapath_task,'zoom_denoise_16k.mat'),'r')
                            wave_rearray_denoise = np.asarray(
                                wave_redata_denoise["zoom"]
                            ).reshape(
                                -1, 1
                            )  # NY668 has two waveform channels
                            ...
                            wave_redata = h5py.File(
                                os.path.join(datapath_task, "zoom_16k.mat"), "r"
                            )
                            wave_rearray = wave_redata["zoom"][:].reshape(
                                -1, 1
                            )  # NY668 has two waveform channels
                        ...
                        wave_re_trim = np.zeros(
                            [
                                ...
                                wave_rearray.shape[1],
                            ]
                        )
                        wave_re_trim_denoise = np.zeros(
                            [
                                int(
                                    ecog.shape[0]
                                    * 1.0
                    ...
                    if not self.FAKE_REPROD:
                        if wave_rearray.shape[0] > wave_re_trim.shape[0]:
                            ...
                            wave_re_trim_denoise = wave_rearray_denoise[
                                : wave_re_trim_denoise.shape[0]
                            ]
                            wave_rearray_denoise = wave_re_trim_denoise
                        else:
                            wave_re_trim[: wave_rearray.shape[0]] = wave_rearray
                            wave_rearray = wave_re_trim
                            wave_re_trim_denoise[
                                : wave_rearray_denoise.shape[0]
                            ] = wave_rearray_denoise
                            wave_rearray_denoise = wave_re_trim_denoise
                    else:
                        wave_rearray = wave_re_trim
                        wave_rearray_denoise = wave_re_trim_denoise
                    ...
                    if cfg.MODEL.WAVE_BASED:
                        ...
                        wave_re_spec_denoise = (
                            wave2spec(
                                torch.tensor(wave_rearray_denoise.T),
                                n_fft=N_FFT_num,
                                noise_db=cfg.MODEL.NOISE_DB,
                                max_db=cfg.MODEL.MAX_DB,
                                power=2 if cfg.MODEL.POWER_SYNTH else 1,
                        ...
                        if self.Wipenoise:
                            if subj is not "NY717" or (
                                task_to_use is not "VisRead"
                                and task_to_use is not "PicN"
                            ):
                                for samples in range(start_ind_re.shape[0]):
                                    if not np.isnan(start_ind_re[samples]):
                                        if samples == 0:
                ...
                try:
                    if HD:
                        ...
                        label_mat = np.array(
                            analysis_event["correctrsp"]
                        )  # analysis_event['correctrsp'][0][:event_range]
                except:
                    # some early samples do not have this column
                    label_mat = [
                        np.repeat("word", start_ind_wave.shape[0]).reshape(-1, 1)[
                            :event_range
                        ]
                    ]
                ...
                for i in range(label_mat.shape[0]):
                    if HD:
                        ...
                        # except:
                            ...
                            label_mati = (
                                label_mat[i][0][0][0].lower().replace(".wav", "")
                            )
            ...
            if not HD:
                ...
                if subj == "NY798":
                    ...
                    bad_samples_ = np.where(
                        np.logical_or(
                            np.logical_or(bad_samples_ == 1, bad_samples_ == 2),
                            bad_samples_ == 4,
                        )
                    )[0]
            if HD:
                bad_channels = (
                    np.array([])
                    if "BadElec" not in subj_param.keys()
                    else subj_param["BadElec"]
                )
            else:
                if HD:
                    bad_channels = (
                    ...
                    try:
                        bad_elecs = scipy.io.loadmat(
                            os.path.join(analysispath_task_, "subj_globals.mat")
                        )["bad_elecs"]
            ...
            if HD:
                ...
                row_nums = min(
                    row_nums, nonzerorownum
                )  # cause there might be some all None rows
                mni_coord = np.stack(
                    [
                        np.array(coord["MNI_x"][:row_nums]),
                        np.array(coord["MNI_y"][:row_nums]),
                        np.array(coord["MNI_z"][:row_nums]),
                    ],
                    axis=1,
                )
                ...
                mni_coord = (
                    mni_coord - np.array([-74.0, -23.0, -20.0])
                ) * 2 / np.array([74.0, 46.0, 54.0]) - 1
                T1_coord = np.stack(
                    [
                        np.array(coord["T1_x"][:row_nums]),
                        np.array(coord["T1_y"][:row_nums]),
                        np.array(coord["T1_z"][:row_nums]),
                    ],
                    axis=1,
                )
                ...
                T1_coord = (T1_coord - np.array([-74.0, -23.0, -20.0])) * 2 / np.array(
                    [74.0, 46.0, 54.0]
                ) - 1
                ...
                percent1 = np.array(
                    [
                        float(coord["AR_Percentage"][i].strip("%").strip()) / 100.0
                ...
                percent2 = np.array(
                    [
                        0.0
                        if isinstance(coord["AR_7"][i], float)
                        else float(coord["AR_7"][i].strip("%").strip()) / 100.0
                        for i in range(row_nums)
                    ]
                )
                percent = np.stack([percent1, percent2], 1)
                AR1 = np.array(
                    [coord["T1_AnatomicalRegion"][i] for i in range(row_nums)]
                )
                AR2 = np.array([coord["AR_8"][i] for i in range(row_nums)])
                AR = np.stack([AR1, AR2], 1)
                regions = np.array(
                    [AR[i, np.argmax(percent, 1)[i]] for i in range(AR.shape[0])]
                )
                regions[
                    np.logical_and(regions == "precentral", mni_coord_raw[:, -1] >= 40)
                ] = "superiorprecentral"
                regions[
                    np.logical_and(regions == "precentral", mni_coord_raw[:, -1] < 40)
                ] = "inferiorprecentral"
                ...
                if self.process_ecog:
                    if self.ReshapeAsGrid:
                        ...
                        if self.low_density:
                            regions = self.rearrange_LD(
                                regions, Crop, mode="region", nums=row_nums
                            )
                            mask = self.rearrange_LD(
                                mask, Crop, mode="mask", nums=row_nums
                            )
                            mni_coord = self.rearrange_LD(
                                mni_coord, Crop, mode="coord", nums=row_nums
                            )
                            mni_coord_raw = self.rearrange_LD(
                                mni_coord_raw, Crop, mode="coord", nums=row_nums
                            )
                            T1_coord = self.rearrange_LD(
                                T1_coord, Crop, mode="coord", nums=row_nums
                            )
                            T1_coord_raw = self.rearrange_LD(
                                T1_coord_raw, Crop, mode="coord", nums=row_nums
                            )
                        else:
                            ...
                            mni_coord_raw = self.rearrange(
                                mni_coord_raw, Crop, mode="coord"
                            )
                            T1_coord = self.rearrange(T1_coord, Crop, mode="coord")
                            T1_coord_raw = self.rearrange(
                                T1_coord_raw, Crop, mode="coord"
                            )
                    else:
                        ...
                        mni_coord_raw = (
                            mni_coord_raw if HD else mni_coord_raw[:lastchannel]
                        )
                        mni_coord = mni_coord if HD else mni_coord[:lastchannel]
                        T1_coord_raw = (
                            T1_coord_raw if HD else T1_coord_raw[:lastchannel]
                        )
            ...
            ecog_ = (
                ecog_ if HD else [ecog_[i][:, :lastchannel] for i in range(len(ecog_))]
            )
            ...
            if not HD:
                ...
                if self.process_ecog:
                    if self.ReshapeAsGrid:
                        ...
                        if self.low_density:
                            # import pdb; pdb.set_trace()
                            ecog_ = self.rearrange_LD(
                                ecog_, Crop, mode="ecog", nums=row_nums
                            )  # conv
                            ecog_raw = self.rearrange_LD(
                                ecog_raw, Crop, mode="ecog", nums=row_nums
                            )
            ...
            if self.process_ecog:
                if subj == "NY742":
                    ...
                    if self.FAKE_LD:
                        ecog_ = np.rot90(
                            ecog_.reshape(-1, 8, 8), k=3, axes=(1, 2)
                        ).reshape(-1, 64)
                    else:
                        ecog_ = np.rot90(
                            ecog_.reshape(-1, 15, 15), k=3, axes=(1, 2)
                        ).reshape(-1, 225)
            ecog_alldataset += [ecog_]
            ecog_raw_alldataset += [ecog_raw]
            baseline_alldataset += [
                (-statics_ecog_zscore[0] / statics_ecog_zscore[1]).reshape([1])
            ]
            # import pdb;pdb.set_trace()
            ...
            start_ind_alldataset += [
                np.concatenate(
                    [
                        np.concatenate(start_ind_train_, axis=0),
                        np.concatenate(start_ind_test_, axis=0),
                    ]
                )
            ]
            start_ind_valid_alldataset += [
                np.concatenate(
                    [
                        np.concatenate(start_ind_valid_train_, axis=0),
                        np.concatenate(start_ind_valid_test_, axis=0),
                    ]
                )
            ]
            start_ind_wave_alldataset += [
                np.concatenate(
                    [
                        np.concatenate(start_ind_wave_down_train_, axis=0),
                        np.concatenate(start_ind_wave_down_test_, axis=0),
                    ]
                )
            ]
            start_ind_wave_valid_alldataset += [
                np.concatenate(
                    [
                        np.concatenate(start_ind_wave_down_valid_train_, axis=0),
                        np.concatenate(start_ind_wave_down_valid_test_, axis=0),
                    ]
                )
            ]
            end_ind_alldataset += [
                np.concatenate(
                    [
                        np.concatenate(end_ind_train_, axis=0),
                        np.concatenate(end_ind_test_, axis=0),
                    ]
                )
            ]
            end_ind_valid_alldataset += [
                np.concatenate(
                    [
                        np.concatenate(end_ind_valid_train_, axis=0),
                        np.concatenate(end_ind_valid_test_, axis=0),
                    ]
                )
            ]
            end_ind_wave_alldataset += [
                np.concatenate(
                    [
                        np.concatenate(end_ind_wave_down_train_, axis=0),
                        np.concatenate(end_ind_wave_down_test_, axis=0),
                    ]
                )
            ]
            end_ind_wave_valid_alldataset += [
                np.concatenate(
                    [
                        np.concatenate(end_ind_wave_down_valid_train_, axis=0),
                        np.concatenate(end_ind_wave_down_valid_test_, axis=0),
                    ]
                )
            ]
            ...
            noise = 10 ** (
                (
                    (np.concatenate(noisesample_, axis=0).mean(0) + 1)
                    / 2
                    * (cfg.MODEL.MAX_DB - cfg.MODEL.NOISE_DB)
                    + cfg.MODEL.NOISE_DB
                )
                / 10
            )
            ...
            if self.Prod:
                ...
                if self.wavebased:
                    ...
                    wave_re_spec_amp_alldataset += [
                        np.concatenate(wave_re_spec_amp_, axis=0)
                    ]
                    wave_re_spec_denoise_alldataset += [
                        np.concatenate(wave_re_spec_denoise_, axis=0)
                    ]
                    wave_re_spec_denoise_amp_alldataset += [
                        np.concatenate(wave_re_spec_denoise_amp_, axis=0)
                    ]
                noise = 10 ** (
                    (
                        (np.concatenate(noisesample_re_, axis=0).mean(0) + 1)
                        / 2
                        * (cfg.MODEL.MAX_DB - cfg.MODEL.NOISE_DB)
                        + cfg.MODEL.NOISE_DB
                    )
                ...
                start_ind_re_alldataset += [
                    np.concatenate(
                        [
                            np.concatenate(start_ind_re_train_, axis=0),
                            np.concatenate(start_ind_re_test_, axis=0),
                        ]
                    )
                ]
                start_ind_re_valid_alldataset += [
                    np.concatenate(
                        [
                            np.concatenate(start_ind_re_valid_train_, axis=0),
                            np.concatenate(start_ind_re_valid_test_, axis=0),
                        ]
                    )
                ]
                start_ind_re_wave_alldataset += [
                    np.concatenate(
                        [
                            np.concatenate(start_ind_re_wave_down_train_, axis=0),
                            np.concatenate(start_ind_re_wave_down_test_, axis=0),
                        ]
                    )
                ]
                start_ind_re_wave_valid_alldataset += [
                    np.concatenate(
                        [
                            np.concatenate(start_ind_re_wave_down_valid_train_, axis=0),
                            np.concatenate(start_ind_re_wave_down_valid_test_, axis=0),
                        ]
                    )
                ]
                end_ind_re_alldataset += [
                    np.concatenate(
                        [
                            np.concatenate(end_ind_re_train_, axis=0),
                            np.concatenate(end_ind_re_test_, axis=0),
                        ]
                    )
                ]
                end_ind_re_valid_alldataset += [
                    np.concatenate(
                        [
                            np.concatenate(end_ind_re_valid_train_, axis=0),
                            np.concatenate(end_ind_re_valid_test_, axis=0),
                        ]
                    )
                ]
                end_ind_re_wave_alldataset += [
                    np.concatenate(
                        [
                            np.concatenate(end_ind_re_wave_down_train_, axis=0),
                            np.concatenate(end_ind_re_wave_down_test_, axis=0),
                        ]
                    )
                ]
                end_ind_re_wave_valid_alldataset += [
                    np.concatenate(
                        [
                            np.concatenate(end_ind_re_wave_down_valid_train_, axis=0),
                            np.concatenate(end_ind_re_wave_down_valid_test_, axis=0),
                        ]
                    )
                ]
                ...
                if self.phoneme:
                    voice_re_alldataset += [
                        np.isin(np.concatenate(phoneme_re_, axis=0), voice)
                    ]
                    unvoice_re_alldataset += [
                        np.isin(np.concatenate(phoneme_re_, axis=0), unvoice)
                    ]
                    semivoice_re_alldataset += [
                        np.isin(np.concatenate(phoneme_re_, axis=0), semivoice)
                    ]
                    plosive_re_alldataset += [
                        np.isin(np.concatenate(phoneme_re_, axis=0), plosive)
                    ]
                    fricative_re_alldataset += [
                        np.isin(np.concatenate(phoneme_re_, axis=0), fricative)
                    ]
            bad_samples_alldataset += [bad_samples_]
            word_alldataset += [
                np.concatenate(
                    [
                        np.concatenate(word_train, axis=0),
                        np.concatenate(word_test, axis=0),
                    ]
                )
            ]
            ...
            label_alldataset += [
                np.concatenate(
                    [
                        np.concatenate(labels_train, axis=0),
                        np.concatenate(labels_test, axis=0),
                    ]
                )
            ]
            ...
            if self.phoneme:
                ...
                unvoice_alldataset += [
                    np.isin(np.concatenate(phoneme_, axis=0), unvoice)
                ]
                semivoice_alldataset += [
                    np.isin(np.concatenate(phoneme_, axis=0), semivoice)
                ]
                plosive_alldataset += [
                    np.isin(np.concatenate(phoneme_, axis=0), plosive)
                ]
                fricative_alldataset += [
                    np.isin(np.concatenate(phoneme_, axis=0), fricative)
                ]

        if self.rearrange_elec:
            # for mask regional based attention
            map_index_dict = np.load(
                "/scratch/xc1490/projects/ecog/ALAE_1023/data/region_index_rearrange_mapping_dict.npy",
                allow_pickle=1,
            ).item()
            ecog_alldataset_new = []
            ...
            for sample in range(len(ecog_alldataset)):
                ...
                mni_coordinate_alldateset_new[sample][
                    map_index_dict[sample][:, 0]
                ] = mni_coordinate_alldateset[sample][map_index_dict[sample][:, 1]]
                mni_coordinate_raw_alldateset_new += [np.zeros([256, 3])]
                mni_coordinate_raw_alldateset_new[sample][
                    map_index_dict[sample][:, 0]
                ] = mni_coordinate_raw_alldateset[sample][map_index_dict[sample][:, 1]]
                ecog_alldataset_new += [
                    np.zeros([ecog_alldataset[sample].shape[0], 256])
                ]
                ecog_alldataset_new[sample][
                    :, map_index_dict[sample][:, 0]
                ] = ecog_alldataset[sample][:, map_index_dict[sample][:, 1]]
                ecog_raw_alldataset_new += [
                    np.zeros([ecog_raw_alldataset[sample].shape[0], 256])
                ]
                ecog_raw_alldataset_new[sample][
                    :, map_index_dict[sample][:, 0]
                ] = ecog_raw_alldataset[sample][:, map_index_dict[sample][:, 1]]
        ...
        if self.low_density:
            self.meta_data = {
                "ecog_alldataset": ecog_alldataset,
                ...
                "formant_re_alldataset": formant_re_alldataset
                if self.formant
                else None,
                "pitch_re_alldataset": pitch_re_alldataset if self.pitch else None,
                "intensity_re_alldataset": intensity_re_alldataset
                if self.intensity
                else None,
            }
        else:
            self.meta_data = {
                ...
                "start_ind_wave_valid_alldataset": start_ind_re_wave_valid_alldataset,
                "spkr_re_alldataset": spkr_re_alldataset,
                "phoneme_re_alldataset": phoneme_re_alldataset
                if self.phoneme
                else None,
                "voice_re_alldataset": voice_re_alldataset,
                "unvoice_re_alldataset": unvoice_re_alldataset,
    ...
    def __len__(self):
        ...
        if self.mode == "train":
            if self.Prod:
                return np.array(
                    [
                        start_ind_re_alldataset.shape[0]
                        * (repeattimes if "NY798" in self.ReqSubjDict else repeattimes)
                        // self.world_size
                        for start_ind_re_alldataset in self.meta_data[
                            "start_ind_re_alldataset"
                        ]
                    ]
                ).sum()
                ...
                return np.array(
                    [
                        start_ind_alldataset.shape[0] * repeattimes
                        for start_ind_alldataset in self.meta_data[
                            "start_ind_alldataset"
                        ]
                    ]
                ).sum()
    ...
    def __getitem__(self, idx):
        ...
        wave_spec_denoise_amp_alldataset = self.meta_data[
            "wave_spec_denoise_amp_alldataset"
        ]
        ...
        if self.Prod:
            ...
            start_ind_re_valid_alldataset = self.meta_data[
                "start_ind_re_valid_alldataset"
            ]
            end_ind_re_valid_alldataset = self.meta_data["end_ind_re_valid_alldataset"]
            start_ind_re_wave_alldataset = self.meta_data[
                "start_ind_re_wave_alldataset"
            ]
            ...
            wave_spec_re_denoise_alldataset = self.meta_data[
                "wave_re_spec_denoise_alldataset"
            ]
            wave_spec_re_denoise_amp_alldataset = self.meta_data[
                "wave_re_spec_denoise_amp_alldataset"
            ]
        ...
        for i in range(num_dataset):
            ...
            if self.mode == "train":
                rand_ind = np.random.choice(
                    np.arange(start_ind_valid_alldataset[i].shape[0])[
                        : -self.TestNum_cum[i]
                    ],
                    1,
                    replace=False,
                ...
                if self.Prod:
                    rand_ind = (
                        idx
                        + start_ind_re_valid_alldataset[i].shape[0]
                        - self.TestNum_cum[i]
                    )
                else:
                    rand_ind = (
                        idx
                        + start_ind_valid_alldataset[i].shape[0]
                        - self.TestNum_cum[i]
                    )
            ...
            ecog_batch = np.zeros(
                (self.SeqLen + n_delay_2 - n_delay_1, ecog_alldataset[i].shape[-1])
            )
            ecog_raw_batch = np.zeros(
                (self.SeqLen + n_delay_2 - n_delay_1, ecog_alldataset[i].shape[-1])
            )
            ...
            wave_batch = np.zeros(
                (
                    (self.SeqLen * int(self.DOWN_WAVE_FS * 1.0 / self.DOWN_ECOG_FS)),
                    wave_alldataset[i].shape[-1],
                )
            )
            if self.wavebased:
                wave_spec_batch = np.zeros(
                    (self.SeqLen, wave_spec_alldataset[i].shape[-1])
                )
            if self.Prod:
                ...
                if self.pre_articulate:
                    ecog_batch_re = np.zeros(
                        (pre_articulate_len, ecog_alldataset[i].shape[-1])
                    )
                    ecog_raw_batch_re = np.zeros(
                        (pre_articulate_len, ecog_alldataset[i].shape[-1])
                    )
                else:
                    ecog_batch_re = np.zeros(
                        (
                            self.SeqLen + n_delay_2 - n_delay_1,
                            ecog_alldataset[i].shape[-1],
                        )
                    )
                    ecog_raw_batch_re = np.zeros(
                        (
                ...
                spkr_batch_re = np.zeros(
                    (self.SeqLenSpkr, spkr_alldataset[i].shape[-1])
                )
                ...
                wave_batch_re = np.zeros(
                    (
                        (
                            self.SeqLen
                            * int(self.DOWN_WAVE_FS * 1.0 / self.DOWN_ECOG_FS)
                        ),
                        wave_alldataset[i].shape[-1],
                    )
                )
                if self.wavebased:
                    wave_spec_batch_re = np.zeros(
                        (self.SeqLen, wave_spec_alldataset[i].shape[-1])
                    )
            ...
            if self.mode == "test" or self.pre_articulate:
                ...
                chosen_start = np.random.choice(
                    np.arange(-64, end_indx - start_indx - 64), 1
                )[0]
                ...
                if self.Prod:
                    ...
                    chosen_start_re = np.random.choice(
                        np.arange(-64, end_indx_re - start_indx_re - 64), 1
                    )[0]
                    indx_re = np.maximum(start_indx_re + chosen_start_re, 0)
                    # indx_re = indx_re-self.ahead_onset_test

            # indx = indx.item()
            ecog_batch = ecog_alldataset[i][
                indx + n_delay_1 : indx + self.SeqLen + n_delay_2
            ]
            ecog_raw_batch = ecog_raw_alldataset[i][
                indx + n_delay_1 : indx + self.SeqLen + n_delay_2
            ]
            ...
            on_stage_batch[
                :,
                np.maximum(start_indx - indx, 0) : np.minimum(
                    end_indx - indx, self.SeqLenSpkr - 1
                ),
            ] = 1.0
            on_stage_wider_batch = np.zeros([1, self.SeqLenSpkr])
            on_stage_wider_batch[
                :,
                np.maximum(start_indx - indx - 5, 0) : np.minimum(
                    end_indx - indx + 5, self.SeqLenSpkr - 1
                ),
            ] = 1.0
            ...
            if self.wavebased:
                ...
                wave_spec_batch_amp = wave_spec_amp_alldataset[i][
                    indx : indx + self.SeqLen
                ]
                wave_spec_batch_denoise = wave_spec_denoise_alldataset[i][
                    indx : indx + self.SeqLen
                ]
                wave_spec_batch_amp_denoise = wave_spec_denoise_amp_alldataset[i][
                    indx : indx + self.SeqLen
                ]
            wave_batch = wave_alldataset[i][
                (indx * int(self.DOWN_WAVE_FS * 1.0 / self.DOWN_ECOG_FS)) : (
                    (indx + self.SeqLen)
                    * int(self.DOWN_WAVE_FS * 1.0 / self.DOWN_ECOG_FS)
                )
            ]
            wave_batch_denoise = wave_denoise_alldataset[i][
                (indx * int(self.DOWN_WAVE_FS * 1.0 / self.DOWN_ECOG_FS)) : (
                    (indx + self.SeqLen)
                    * int(self.DOWN_WAVE_FS * 1.0 / self.DOWN_ECOG_FS)
                )
            ...
            if self.Prod:
                ...
                on_stage_re_batch[
                    :,
                    np.maximum(start_indx_re - indx_re, 0) : np.minimum(
                        end_indx_re - indx_re, self.SeqLenSpkr - 1
                    ),
                ] = 1.0
                on_stage_wider_re_batch = np.zeros([1, self.SeqLenSpkr])
                on_stage_wider_re_batch[
                    :,
                    np.maximum(start_indx_re - indx_re - 5, 0) : np.minimum(
                        end_indx_re - indx_re + 5, self.SeqLenSpkr - 1
                    ),
                ] = 1.0
                if self.pre_articulate:
                    ecog_batch_re = ecog_alldataset[i][
                        indx_re : indx_re + pre_articulate_len
                    ]
                    ecog_raw_batch_re = ecog_raw_alldataset[i][
                        indx_re : indx_re + pre_articulate_len
                    ]
                else:
                    ecog_batch_re = ecog_alldataset[i][
                        indx_re + n_delay_1 : indx_re + self.SeqLen + n_delay_2
                    ]
                    ecog_raw_batch_re = ecog_raw_alldataset[i][
                        indx_re + n_delay_1 : indx_re + self.SeqLen + n_delay_2
                    ]
                    # ecog_batch_re = ecog_alldataset[i][indx_re+n_delay_1:indx_re+self.SeqLen+n_delay_1]
                spkr_batch_re = spkr_re_alldataset[i][
                    indx_re : indx_re + self.SeqLenSpkr
                ]
                if self.phoneme:
                    phoneme_batch_re = phoneme_re_alldataset[i][
                        indx_re : indx_re + self.SeqLenSpkr
                    ]

                    voice_batch_re = voice_re_alldataset[i][
                        indx_re : indx_re + self.SeqLenSpkr
                    ]
                    unvoice_batch_re = unvoice_re_alldataset[i][
                        indx_re : indx_re + self.SeqLenSpkr
                    ]
                    semivoice_batch_re = semivoice_re_alldataset[i][
                        indx_re : indx_re + self.SeqLenSpkr
                    ]
                    plosive_batch_re = plosive_re_alldataset[i][
                        indx_re : indx_re + self.SeqLenSpkr
                    ]
                    fricative_batch_re = fricative_re_alldataset[i][
                        indx_re : indx_re + self.SeqLenSpkr
                    ]
                if self.formant:
                    formant_batch_re = formant_re_alldataset[i][
                        indx_re : indx_re + self.SeqLenSpkr
                    ]
                if self.pitch:
                    pitch_batch_re = pitch_re_alldataset[i][
                        indx_re : indx_re + self.SeqLenSpkr
                    ]
                if self.intensity:
                    intensity_batch_re = intensity_re_alldataset[i][
                        indx_re : indx_re + self.SeqLenSpkr
                    ]

                if self.wavebased:
                    wave_spec_batch_re = wave_spec_re_alldataset[i][
                        indx_re : indx_re + self.SeqLen
                    ]
                    wave_spec_batch_amp_re = wave_spec_re_amp_alldataset[i][
                        indx_re : indx_re + self.SeqLen
                    ]
                    wave_spec_batch_re_denoise = wave_spec_re_denoise_alldataset[i][
                        indx_re : indx_re + self.SeqLen
                    ]
                    wave_spec_batch_re_amp_denoise = (
                        wave_spec_re_denoise_amp_alldataset[i][
                            indx_re : indx_re + self.SeqLen
                        ]
                    )
                wave_batch_re = wave_re_alldataset[i][
                    (indx_re * int(self.DOWN_WAVE_FS * 1.0 / self.DOWN_ECOG_FS)) : (
                        (indx_re + self.SeqLen)
                        * int(self.DOWN_WAVE_FS * 1.0 / self.DOWN_ECOG_FS)
                    )
                ]
                wave_batch_re_denoise = wave_re_denoise_alldataset[i][
                    (indx_re * int(self.DOWN_WAVE_FS * 1.0 / self.DOWN_ECOG_FS)) : (
                        (indx_re + self.SeqLen)
                        * int(self.DOWN_WAVE_FS * 1.0 / self.DOWN_ECOG_FS)
                    )
            ...
            if self.wavebased:
                ...
                wave_spec_denoise_batch_all += [
                    wave_spec_batch_denoise[np.newaxis, ...]
                ]
                wave_spec_denoise_amp_batch_all += [
                    wave_spec_batch_amp_denoise[np.newaxis, ...]
                ]
            ...
            if self.Prod:
                ...
                if self.wavebased:
                    ...
                    wave_spec_re_amp_batch_all += [
                        wave_spec_batch_amp_re[np.newaxis, ...]
                    ]
                    wave_spec_re_denoise_batch_all += [
                        wave_spec_batch_re_denoise[np.newaxis, ...]
                    ]
                    wave_spec_re_denoise_amp_batch_all += [
                        wave_spec_batch_re_amp_denoise[np.newaxis, ...]
                    ]
            ...
            gender_all += [
                ...
                if self.meta_data["gender_alldataset"][i] == "Male"
        ...
        if self.wavebased:
            ...
            wave_spec_denoise_batch_all = np.concatenate(
                wave_spec_denoise_batch_all, axis=0
            )
            wave_spec_denoise_amp_batch_all = np.concatenate(
                wave_spec_denoise_amp_batch_all, axis=0
            )
        ...
        if self.Prod:
            ...
            if self.wavebased:
                ...
                wave_spec_re_amp_batch_all = np.concatenate(
                    wave_spec_re_amp_batch_all, axis=0
                )
                wave_spec_re_denoise_batch_all = np.concatenate(
                    wave_spec_re_denoise_batch_all, axis=0
                )
                wave_spec_re_denoise_amp_batch_all = np.concatenate(
                    wave_spec_re_denoise_amp_batch_all, axis=0
                )
            wave_re_batch_all = np.concatenate(wave_re_batch_all, axis=0)
            wave_re_denoise_batch_all = np.concatenate(
                wave_re_denoise_batch_all, axis=0
            )
            on_stage_re_batch_all = np.concatenate(on_stage_re_batch_all, axis=0)
            on_stage_wider_re_batch_all = np.concatenate(
                on_stage_wider_re_batch_all, axis=0
            )
        label_batch_all = np.concatenate(label_batch_all, axis=0).tolist()
        word_batch_all = np.array(word_batch_all)
        baseline_batch_all = np.concatenate(
            self.meta_data["baseline_alldataset"], axis=0
        )
        mni_coordinate_all = np.concatenate(mni_coordinate_all, axis=0)
