...
class LAE(nn.Module):
    ...
    def forward(self, rec, spec, tracker=None, reweight=1):
        if self.db:
            loss_db = self.ghm_db(rec.to(device), spec.to(device), torch.ones(spec.shape).to(device),\
                reweight=reweight.to(device))
...
class Model(nn.Module):
    ...
    def run_components_loss(
        ...
        spec_amp = amplitude(spec, self.noise_db, self.max_db).transpose(-2, -1).to(device)
        rec_amp = amplitude(rec, self.noise_db, self.max_db).transpose(-2, -1).to(device)
        spec_mel = to_db(
            torchaudio.transforms.MelScale(f_max=8000, n_stft=self.n_fft).to(device)(
                spec_amp
            ).transpose(-2, -1),
            self.noise_db,
            self.max_db,
                ...
                if key in ["amplitudes"]:
                    ...
                    if self.ghm_loss:
                        diff = (
                            alpha["amplitudes"]
                            * 540
                            * self.lae3(tmp_target.to(device), tmp_ecog.to(device), reweight=weight.to(device))
                        )
                    else:
                        diff = (
                            alpha["amplitudes"]
    ...
    def forward(
        ...
        if ae:  # audio to audio
            ...
            if IG_Viz:
                return torch.Tensor([Loss]).unsqueeze(0).requires_grad_()
            else:
                return Loss, tracker
