...
class WindowAttention3D(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        #    attn = self.softmax(attn)
        if self.causal:
            attn = attn.view(B_, self.num_heads, N, N) + mask_causal.unsqueeze(
                0
            ).unsqueeze(1)
        attn = self.softmax(attn)
...
if __name__ == "__main__":
    ...
    swin_transformer = SwinTransformer3D(
        ...
        num_heads=[3, 6, 12, 24],
        window_size=(
            window_size_Ts[rnn_layers],
            window_size_Es[rnn_layers][0],
            window_size_Es[rnn_layers][1],
        ),
        drop_rate=0.2,
        attn_drop_rate=0.2,
        mlp_ratio=4.0,
