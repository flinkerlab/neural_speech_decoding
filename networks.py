...
def wave2spec(wave,n_fft=256,wave_fr=16000,spec_fr=125,noise_db=-60,max_db=35,to_db=True,power=2):
   if to_db:
...
def inverse_spec_to_audio(spec,n_fft = 511,win_length = 511,hop_length = 128,power_synth=True):
   ...
   return torch.istft(spec_complex, n_fft=n_fft, window=window, center=True, win_length=win_length, hop_length=hop_length)
...
def interp(y, xs):
   y = y[0].squeeze()
   y = F.pad(y,[1,1])
   x = torch.linspace(-1.,1.,y.shape[0]-2)
...
class LearntFormantFilter(nn.Module):
   def __init__(self, nfft,nfilter=50,max_filter_freq=2000.,max_frequency=8000.,cubic=False,noise=False,dynamic=False,learnedbandwidth=False,temporal_invariant=True,reverse_order=True):
      ...
      if dynamic:
         ...
         self.net_learnx = Comp_Spec_CNN_learnx_fromspec(hidden_dim= nfft, n_fft=self.nfilter,reverse_order=reverse_order)#input is spectrogram
      else:
         self.x = torch.nn.Parameter(torch.ones(1, 1, 1, self.nfilter)) # for dynamic filter shape  B x 1 x T x nfilter
   ...
   def forward(self, fi, bi, f_sampling = None,spec = None):
      ...
      x1_3db_idx = x1_3db_idxs[0,0,0,0].float() + x1_3db_values0/(x1_3db_values0+x1_3db_values1)*(x1_3db_idxs[0,0,0,1].float()-x1_3db_idxs[0,0,0,0].float())
      ...
      x2_3db_idx = x2_3db_idxs[0,0,0,0].float() + x2_3db_values0/(x2_3db_values0+x2_3db_values1)*(x2_3db_idxs[0,0,0,1].float()-x2_3db_idxs[0,0,0,0].float())
      # Calculate b_x
      b_x = (self.nfilter//2-1 - x1_3db_idx + x2_3db_idx).float()/self.nfilter * self.max_filter_freq
      if f_sampling is None:
         grid_x,grid_y = torch.meshgrid(torch.arange(self.nfft).float(),torch.zeros(fi.shape[2]))
         grid_x = grid_x.unsqueeze(0) # 1 x nfft x T
         grid_y = grid_y.unsqueeze(0)
      else:
         grid_x = f_sampling/self.max_frequency*self.nfft
         grid_y = torch.zeros_like(grid_x)
      a = (2*b_x/self.max_filter_freq) / ((bi+1E-8)/self.max_frequency*self.nfft)
      b = -2*b_x/self.max_filter_freq * fi/(bi+1E-8)
      grid_x = grid_x*a+b
      grid = torch.stack([grid_x,grid_y.repeat([fi.shape[0],1,1])],-1)
      if not self.dynamic:
         x_cum = x_cum.repeat([fi.shape[0],1,1,1]) # XUPENG: delete this line if use dynamic filter shape
      if self.cubic:
         ...
         sampled_filter = F.grid_sample(x_cum,grid,align_corners=None) #B x 1 x self.nfft x T
...
class LearntFormantFilterMultiple(nn.Module):
   def __init__(self, n_formants,n_formants_noise,nfft,nfilter=50,max_filter_freq=2000.,max_frequency=8000.,dynamic=False,learnedbandwidth=False,temporal_invariant=True,reverse_order=True):
      ...
      for i in range(n_formants):
         self.filter.append(LearntFormantFilter(nfft=nfft,nfilter=nfilter,max_filter_freq=max_filter_freq,max_frequency=max_frequency,noise=False,dynamic=dynamic,learnedbandwidth=learnedbandwidth,temporal_invariant=temporal_invariant,reverse_order=reverse_order))

      self.filter_noise = nn.ModuleList([])
      for i in range(n_formants_noise):
         self.filter_noise.append(LearntFormantFilter(nfft=nfft,nfilter=nfilter,max_filter_freq=max_filter_freq,max_frequency=max_frequency,noise=True,dynamic=dynamic,learnedbandwidth=learnedbandwidth,temporal_invariant=temporal_invariant,reverse_order=reverse_order))
...
#       import pdb; pdb.set_trace();
      ...
      combined_filter = torch.stack([self.filter[i](fi[i].transpose(-1,-2), bi[i].transpose(-1,-2), f_sampling = f_sampling, spec = spec) for i in range(self.n_formants)],-1) #B x 1 x T x nfft x n_formants
      ...
      if not is_harm:
         ...
         #    combined_filter = combined_filter.squeeze(1) # B x T x nfft x n_formants
         combined_filter_noise = torch.stack([self.filter_noise[i](fi[i+self.n_formants].transpose(-1,-2),bi[i+self.n_formants].transpose(-1,-2),f_sampling=f_sampling, spec = spec) for i in range(self.n_formants_noise)],-1) #B x 1 x T x nfft x n_formants
         combined_filter_noise = combined_filter_noise.squeeze(1) # B x T x nfft x n_formants
...
class DoubleConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels,  mid_channels=None,groups=16, causal=0,anticausal=0):
        super().__init__()
        if not mid_channels:
            mid_channels = out_channels
        self.double_conv = nn.Sequential(
            ln_c.Conv1d(in_channels,  mid_channels,3,1,1 ,causal=causal,anticausal = anticausal),
            GroupNormXDim(groups,mid_channels),
            nn.LeakyReLU(),
            ln_c.Conv1d(mid_channels, out_channels,3,1,1 ,causal=causal,anticausal = anticausal),
...
class Upsample_Block(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, bilinear=False,causal=False, anticausal=False):
        super().__init__()
        ...
        if bilinear:
            ...
            self.up = ln_c.ConvTranspose1d(in_channels,out_channels, 3, 2, 1, transform_kernel=True ,causal=causal,anticausal = anticausal)
    def forward(self, x ):
        return   self.up(x)
...
class FormantSysth(nn.Module):
   def __init__(self, n_mels=64, k=100, wavebased=False,n_fft=256,noise_db=-50,max_db=22.5,
               dbbased=False,add_bgnoise=True,log10=False,noise_from_data=False,return_wave=False,
               power_synth=False, n_formants=6,normed_mask=False, dummy_formant=False, learned_mask=False,
               n_filter_samples=40,dynamic_filter_shape=False,learnedbandwidth=False,return_filtershape=False,spec_fr=125,reverse_order=True):#dynamic_bgnoise=False,
    ...
    #   self.silient = Parameter(torch.Tensor(1,1,n_mels))
      ...
      with torch.no_grad():
         ...
         if self.dummy_formant:
            nn.init.constant_(self.dummy_f,1.)
            nn.init.constant_(self.dummy_w,1.)
            nn.init.constant_(self.dummy_a,-1.)
      ...
      if learned_mask:
         ...
         if learnedbandwidth or dynamic_filter_shape:
            self.formant_masks_hamon = LearntFormantFilterMultiple(n_formants,0,n_fft,n_filter_samples,2000.,8000.,dynamic = dynamic_filter_shape,learnedbandwidth=learnedbandwidth,reverse_order=reverse_order)
            self.formant_masks_noise = LearntFormantFilterMultiple(n_formants,1,n_fft,n_filter_samples,2000.,8000.,dynamic = dynamic_filter_shape,learnedbandwidth=learnedbandwidth,reverse_order=reverse_order) #only for hamonic? or both?
         else:  #no learned bandwith with current setting!
            self.formant_masks = LearntFormantFilterMultiple(n_formants,1,n_fft,n_filter_samples,2000.,8000.,reverse_order=reverse_order)
         if dummy_formant:
            self.formant_masks_dummy = LearntFormantFilterMultiple(1,0,n_fft,n_filter_samples,2000.,8000.,dynamic=False,reverse_order=reverse_order)
      print ('n filter samples!!', n_filter_samples)
   
   def formant_mask(self,freq_hz,bandwith_hz,amplitude,linear=False, triangle_mask = False,duomask=True, n_formant_noise=1,f0_hz=None,noise=False,learned_mask=None,spec = None):
      ...
      if self.return_wave: #False in train, True in testp
         ...
         if self.learned_mask:
            hamonic_dist = (amplitude*learned_mask(freq_hz,bandwith_hz+0.01,k_f0, spec = spec)).sqrt().sum(-1).permute([0,2,1]) #BxkxT
         else:
            hamonic_dist = (amplitude*torch.exp(-((k_f0-freq_hz))**2/(2*(bandwith_hz/alpha+0.01)**2))).sqrt().sum(-1).permute([0,2,1]) #BxkxT
         if self.dummy_formant:
            if self.learned_mask:
               dummy = dummy_amp*self.formant_masks_dummy(dummy_freq,dummy_band,k_f0, spec = spec).sum(-1).permute([0,2,1]) # sum of power
            else:
               dummy = dummy_amp*torch.exp(-0.5*(((k_f0-dummy_freq))**2/((dummy_band/(2*(2*np.log(2))**(0.5))+0.01)**2)+1E-8)).sum(-1).permute([0,2,1]) # sum of power
      ...
      if triangle_mask:
         ...
         if duomask:
            ...
            if self.learned_mask:
               #print ('learned_mask')
               masks_hamon = amplitude[...,:-n_formant_noise]*learned_mask(freq_hz[...,:-n_formant_noise],bandwith_hz[...,:-n_formant_noise]+0.01, spec = spec)
               bw = bandwith_hz[...,-n_formant_noise:]
               masks_noise = amplitude[...,-n_formant_noise:] * learned_mask(freq_hz[...,-n_formant_noise:]+0.01,bw, spec = spec)
            else:
            ...
            if self.learned_mask:
               masks_hamon = amplitude*learned_mask(freq_hz,bandwith_hz+0.01, spec = spec)
            else:
         ...
         if self.power_synth:
            ...
            if self.learned_mask:
               ...
               masks = amplitude*learned_mask(freq_hz,bandwith_hz+0.01, spec = spec)
            else:
               masks = amplitude*torch.exp(-((grid_freq_hz-freq_hz))**2/(2*(bandwith_hz/alpha+0.01)**2))
            if self.dummy_formant and not noise:
               if self.learned_mask:
                  dummy = dummy_amp*self.formant_masks_dummy(dummy_freq,dummy_band+0.01, spec = spec)
               else:
                  dummy = dummy_amp*torch.exp(-0.5*(((grid_freq_hz-dummy_freq))**2/((dummy_band/(2*(2*np.log(2))**(0.5))+0.01)**2)+1E-8))
               masks = torch.cat([dummy.expand(masks.shape[0],masks.shape[1],-1,-1),masks],-1)
         else:
            if self.learned_mask:
               masks = amplitude*learned_mask(freq_hz,bandwith_hz+0.01, spec = spec).sqrt()
            else:
   ...
   def forward(self,components,onstage,enable_hamon_excitation=True,enable_noise_excitation=True,enable_bgnoise=True,n_iter=0,save_path=''):
      ...
      if not self.noise_from_data:
          noise_dist_learned = components['noisebg'].unsqueeze(dim=-1).unsqueeze(dim=-1).permute(0,3,2,1)  # torch.Size([1, 1, 1, 256])
      if self.wavebased:
      ...
      if self.learned_mask:
         if self.learnedbandwidth or self.dynamic_filter_shape:
            ...
            self.mask_hamon = self.formant_mask(components['freq_formants_hamon_hz'],components['bandwidth_formants_hamon_hz'],components['amplitude_formants_hamon'],\
                linear = self.linear_scale,f0_hz = f0_hz,learned_mask=self.formant_masks_hamon, spec = spec_smooth)
            #print ('self.mask_noise formant_mask')
            self.mask_noise = self.formant_mask(components['freq_formants_noise_hz'],components['bandwidth_formants_noise_hz'],components['amplitude_formants_noise'],\
                linear = self.linear_scale, triangle_mask= False if self.wavebased else True, duomask = duomask, n_formant_noise = n_formant_noise, f0_hz = f0_hz, noise = True,\
                    learned_mask=self.formant_masks_noise, spec = spec_smooth)
         else:
            self.mask_hamon = self.formant_mask(components['freq_formants_hamon_hz'],components['bandwidth_formants_hamon_hz'],components['amplitude_formants_hamon'],linear = self.linear_scale,f0_hz = f0_hz,learned_mask=self.formant_masks)
            self.mask_noise = self.formant_mask(components['freq_formants_noise_hz'],components['bandwidth_formants_noise_hz'],components['amplitude_formants_noise'],linear = self.linear_scale,triangle_mask=False if self.wavebased else True,duomask=duomask,n_formant_noise=n_formant_noise,f0_hz = f0_hz,noise=True,learned_mask=self.formant_masks)
      else:
         self.mask_hamon = self.formant_mask(components['freq_formants_hamon_hz'],components['bandwidth_formants_hamon_hz'],components['amplitude_formants_hamon'],\
             linear = self.linear_scale,f0_hz = f0_hz)
         self.mask_noise = self.formant_mask(components['freq_formants_noise_hz'],components['bandwidth_formants_noise_hz'],components['amplitude_formants_noise'],\
             linear = self.linear_scale,triangle_mask=False if self.wavebased else True,duomask=duomask,n_formant_noise=n_formant_noise,f0_hz = f0_hz,noise=True)
      ...
      if n_iter != 0:
         if self.return_filtershape:
             ...
             np.save(save_path + '/filtershape/hamon_{}'.format(n_iter),self.mask_hamon.detach().cpu().numpy())
             np.save(save_path + '/filtershape/noise_{}'.format(n_iter),self.mask_noise.detach().cpu().numpy())
      ...
      bgdist = F.softplus(self.bgnoise_amp) * self.noise_dist if self.noise_from_data else \
            noise_dist_learned  # torch.Size([1, 1, 1, 256])
      ...
      if self.wavebased:
         ...
         speech = ((self.excitation_hamon*self.mask_hamon_sum) if enable_hamon_excitation else torch.zeros(self.excitation_hamon.shape))\
             + (self.noise_excitation if enable_noise_excitation else 0) + bgn
        ...
        ( ( (bgdist*self.bgnoise*0.0003) if not self.dbbased else ( 2*torchaudio.transforms.AmplitudeToDB()(bgdist*0.0003)/35. + self.bgnoise) ) if (self.add_bgnoise and enable_bgnoise) else 0)\
            + (self.silient*torch.ones(self.mask_hamon_sum.shape) if self.dbbased else 0)
         if not self.dbbased:
            speech = db(speech)
      if self.return_wave:
...
class FormantEncoder(nn.Module):
   def __init__(self, n_mels=64, n_formants=4,n_formants_noise=2,min_octave=-31,max_octave=96,wavebased=False,n_fft=256,noise_db=-50,max_db=22.5,\
                            broud=True,power_synth=False,hop_length=128,patient='NY742', gender_patient = 'Female',larger_capacity=False,unified=False):
      ...
      if unified:
         self.formant_freq_limits_abs = torch.tensor([950.,3400.,3800.,5000.,6000.,7500.]).reshape([1,6,1]) #freq difference
         self.formant_freq_limits_abs_low = torch.tensor([200.,500.,1400.,3000,4000.,4500.]).reshape([1,6,1]) #freq difference
         self.formant_freq_limits_abs_male = torch.tensor([950.,3400.,3800.,5000.,6000.,7500.]).reshape([1,6,1]) #freq difference
         self.formant_freq_limits_abs_low_male = torch.tensor([200.,500.,1400.,3000,4000.,4500.]).reshape([1,6,1]) #freq difference
      else:
         ...
         self.formant_freq_limits_abs_male = torch.tensor([850.,3000.,3400.,4600.,6000.,7500.]).reshape([1,6,1]) #freq difference
         self.formant_freq_limits_abs_low_male = torch.tensor([200.,500.,1400.,3000,4000.,4500.]).reshape([1,6,1]) #freq difference
      ...
      if broud:
         ...
         if gender_patient == 'Female':
             self.conv1_narrow = nn.Sequential(ln.Conv1d(n_fft,64,3,1,1),
                                 nn.GroupNorm(32,64), nn.LeakyReLU(0.2),
                                 ln.Conv1d(64,128,3,1,1),nn.GroupNorm(32,128),
                                       nn.LeakyReLU(0.2))
         else:
             if larger_capacity:
               self.conv1_narrow = nn.Sequential(ln.Conv1d(n_fft,256,5,1,2),  #add capacity for male!!
                                 nn.GroupNorm(32,256), nn.LeakyReLU(0.2),
                                 ln.Conv1d(256,128,5,1,2),  
                                 nn.GroupNorm(32,128), nn.LeakyReLU(0.2),
                                 ln.Conv1d(128,64,5,1,2),
                                 nn.GroupNorm(32,64), nn.LeakyReLU(0.2),
                                 ln.Conv1d(64,128,5,1,2),nn.GroupNorm(32,128),
                                       nn.LeakyReLU(0.2))
             else:
               self.conv1_narrow = nn.Sequential(ln.Conv1d(n_fft,128,3,1,1),  #add capacity for male!!
                                 nn.GroupNorm(32,128), nn.LeakyReLU(0.2), #0518  temporally change back to 3,1,1 from 5,1,2
      ...
      if gender_patient == 'Female':
         self.conv1 = nn.Sequential(ln.Conv1d(n_fft,64,3,1,1),
                                nn.GroupNorm(32,64), nn.LeakyReLU(0.2),
                                ln.Conv1d(64,128,3,1,1),nn.GroupNorm(32,128),
                                    nn.LeakyReLU(0.2))
      else:
         self.conv1 = nn.Sequential(ln.Conv1d(n_fft,128,3,1,1),  #add capacity for male!!
                                nn.GroupNorm(32,128), nn.LeakyReLU(0.2),
                                ln.Conv1d(128,64,3,1,1),
                                nn.GroupNorm(32,64), nn.LeakyReLU(0.2),
                                ln.Conv1d(64,128,3,1,1),nn.GroupNorm(32,128),
      ...
      print ('encoder loudness,',n_fft,)
      
      self.conv_loudness = nn.Sequential(ln.Conv1d(n_fft if wavebased else n_mels,128,1,1,0),  #add capacity for male? or maybe not?
                                       nn.LeakyReLU(0.2),
      ...
      self.conv2_bgnoise = ln.Conv1d(128, n_fft if wavebased else n_mels, 1, 1, 0)  #add capacity for male?
      self.norm1_bgnoise = nn.GroupNorm(32, 128 if wavebased else n_mels)
      self.norm2_bgnoise = nn.GroupNorm(32, n_fft if wavebased else n_mels)  #add capacity for male!!
      
      self.amplifier = Parameter(torch.Tensor(1))
      self.bias = Parameter(torch.Tensor(1))
      with torch.no_grad():
         nn.init.constant_(self.amplifier,1.0)
         nn.init.constant_(self.bias,-50.)
      print ('self.noise_db,self.max_db',self.noise_db,self.max_db)
   def forward(self,x,x_denoise=None,duomask=False,noise_level = None,x_amp=None,gender='Female',onstage=None):
      ...
      x_smooth = F.conv2d(x.unsqueeze(1).transpose(-2,-1),hann_win,padding=[(win-1)//2,0]).transpose(-2,-1).squeeze(1)
      if self.power_synth:
      ...
      if self.broud:
         ...
         noisebg = F.softmax(self.norm1_bgnoise(self.conv1_bgnoise(x_common_narrow)),dim=1)
         noisebg = F.softmax(self.norm2_bgnoise(self.conv2_bgnoise(noisebg)),dim=1).max(-1)[0]
         ...
         if self.unified: #we unify f0 learning of male and female
            f0_hz = torch.sigmoid(self.conv_f0_narrow(torch.cat([x_fundementals_narrow,x_fundementals_mel],dim=1))) * 220 + 80      #80-300
         else:
            f0_hz = torch.sigmoid(self.conv_f0_narrow(torch.cat([x_fundementals_narrow,x_fundementals_mel],dim=1))) * 120 + 80 + 100*gender.unsqueeze(-1) # 80hz < f0 < 200 hz for male, 180hz < f0 < 300 hz for female
         f0 = torch.clamp(mel_scale(self.n_mels,f0_hz)/(self.n_mels*1.0),min=0.0001)
         win = 20*self.n_fft//256+1
         hann_win = torch.hann_window(win,periodic=False).reshape([1,1,win,1])
         x = to_db(F.conv2d(x_amp,hann_win,padding=[(win-1)//2,0]).transpose(-2,-1),self.noise_db,self.max_db).squeeze(1)
      ...
      formants_freqs_hz_male = formants_freqs*(self.formant_freq_limits_abs_male[:,:self.n_formants]-self.formant_freq_limits_abs_low_male[:,:self.n_formants])+self.formant_freq_limits_abs_low_male[:,:self.n_formants]
      formants_freqs_hz_male_female = torch.stack([formants_freqs_hz_male,formants_freqs_hz],dim=-1)
      dummy = gender.unsqueeze(-1).unsqueeze(-1).expand(gender.size(0), formants_freqs_hz.size(1), formants_freqs_hz.size(2),2)
      formants_freqs_hz = formants_freqs_hz_male_female.gather(-1,dummy.long())[...,0]
      formants_freqs = torch.clamp(mel_scale(self.n_mels,formants_freqs_hz)/(self.n_mels*1.0),min=0) 
      formants_bandwidth_hz = 0.65*(0.00625*torch.relu(formants_freqs_hz)+375) # production
      formants_bandwidth_hz_male = 0.55*0.65*(0.00625*torch.relu(formants_freqs_hz)+300) 
      formants_bandwidth_hz_male_female = torch.stack([formants_bandwidth_hz_male,formants_bandwidth_hz],dim=-1)
      dummy = gender.unsqueeze(-1).unsqueeze(-1).expand(gender.size(0), formants_bandwidth_hz.size(1), formants_bandwidth_hz.size(2),2)
      formants_bandwidth_hz = formants_bandwidth_hz_male_female.gather(-1,dummy.long())[...,0]
...
class FromECoG_causal(nn.Module):
    def __init__(self, outputs,residual=False,shape='3D',causal=False,anticausal = False,in_channel=1):
        super().__init__()
        self.residual=residual
        if shape =='3D':
            self.from_ecog = ln_c.Conv3d(in_channel, outputs, [(5 if causal else 9),1,1], 1, [4,0,0],causal=causal,anticausal = anticausal)
        else:
            self.from_ecog = ln_c.Conv2d(in_channel, outputs, [(5 if causal else 9),1], 1, [4,0],causal=causal,anticausal = anticausal)
...
class ECoGMappingBlock_causal(nn.Module):
    def __init__(self, inputs, outputs, kernel_size,dilation=1,fused_scale=True,residual=False,resample=[],pool=None,shape='3D',causal=False,anticausal=False,norm = nn.GroupNorm):
      ...
      if pool is None:
         ...
         self.conv1 = conv(inputs, outputs, kernel_size, 1, self.padding, dilation=dilation, bias=False,causal=causal,anticausal = anticausal)
      ...
      if self.inputs_resample or self.dim_missmatch:
         if pool is None:
               ...
               self.convskip = conv(inputs, outputs, kernel_size, 1, self.padding, dilation=dilation, bias=False,causal=causal,anticausal = anticausal)
...
class BasicRNN(torch.nn.Module):
    ...
    def __init__(
        ...
        # define architecture
        self.transform_initial_state = torch.nn.Linear(self.n_input_features,
                                                       self.n_rnn_units * self.n_directions * self.n_layers, bias=False)
        self.rnn = torch.nn.LSTM(self.n_input_features, self.n_rnn_units,self.n_layers,
                                 batch_first=self.batch_first, dropout=dropout, bidirectional=bidirectional)
        if self.use_final_linear_layer:
            self.output_layer = torch.nn.Linear(n_rnn_units * self.n_directions, self.n_output_features)

    def forward(self, inputs, initial_state=None, seq_lengths=None, transform_state=False, **kwargs):
        ...
        if initial_state is not None and transform_state:
            ...
            transformed_state = transformed_state.view(-1, self.n_layers * self.n_directions, self.n_rnn_units)
        else:
            transformed_state = initial_state

        # run rnn
        if seq_lengths is not None:
            inputs = torch.nn.utils.rnn.pack_padded_sequence(inputs, seq_lengths, batch_first=True, enforce_sorted=False)
        #import pdb; pdb.set_trace()
        output, state = self.rnn(inputs, transformed_state)
        if seq_lengths is not None:
            output, _ = torch.nn.utils.rnn.pad_packed_sequence(output, batch_first=self.batch_first, total_length=self.max_length)
...
class Speaker_Classifier_GR(torch.nn.Module):
    def __init__(self, Adversarial_Speaker_Weight = 0.0005, Channels = [256], previous_Channels = 256, Num_Speakers = 2):
