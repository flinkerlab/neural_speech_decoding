...
parser.add_argument('--subject',  type=str, default='NY668',
...
parser.add_argument('--OUTPUT_DIR',  type=str, default='output/ecog_a2a',
                help='OUTPUT_DIR')
...
parser.add_argument('--unified',  type=int, default=0,
...
parser.add_argument('--BIDIRECTION',  type=int,default=1,
                help='BIDIRECTION')
parser.add_argument('--MAPPING_FROM_ECOG',  type=str,default='ECoGMappingBottleneck_ran',
...
def train(cfg, logger, local_rank, world_size, distributed):
    ...
    if args_.pretrained_model_dir is '':
        if allsubj_param['Subj'][train_subject_info[0]]['Gender'] == "Female": #cfg.DATASET.SUBJECT is a list
            load_model_dir = './training_artifacts/742_han5amppowerloss_alphasup/model_epoch31.pth'
    ...
    extra_checkpoint_data = checkpointer.load(ignore_last_checkpoint=True if DEBUG else cfg.IGNORE_LOADING,ignore_auxiliary=True,file_name=load_model_dir)
    ...
    print ('supervisions: args_.formant_supervision, args_.pitch_supervision, args_.intensity_supervision', args_.formant_supervision, args_.pitch_supervision, args_.intensity_supervision)
    ...
    if args_.cv :
        dataset = TFRecordsDataset(cfg, logger, rank=local_rank, world_size=world_size, buffer_size_mb=1024, channels=cfg.MODEL.CHANNELS,param=param,allsubj_param=allsubj_param,\
                    SUBJECT = train_subject_info, ReshapeAsGrid=1, rearrange_elec=0, low_density = (cfg.DATASET.DENSITY == 'LD'), process_ecog = False, \
                    formant_label = args_.formant_supervision, pitch_label = pitch_label, intensity_label = intensity_label,DEBUG=DEBUG,LOO=args_.LOO,cv_ind = args_.cv_ind,use_denoise=args_.use_denoise)
    else:
        dataset = TFRecordsDataset(cfg, logger, rank=local_rank, world_size=world_size, buffer_size_mb=1024, channels=cfg.MODEL.CHANNELS,param=param,allsubj_param=allsubj_param,\
                    SUBJECT = train_subject_info, ReshapeAsGrid=1, rearrange_elec=0, low_density = (cfg.DATASET.DENSITY == 'LD'), process_ecog = False, \
                    formant_label = args_.formant_supervision, pitch_label = pitch_label, intensity_label = intensity_label,DEBUG=DEBUG,use_denoise=args_.use_denoise)

    dataset_test_all = {}
    #print ('test_sub infor', test_subject_info)
    for subject in test_subject_info:
        if args_.cv :
            dataset_test_all[subject] = TFRecordsDataset(cfg, logger, rank=local_rank, world_size=world_size, buffer_size_mb=1024, channels=cfg.MODEL.CHANNELS,train=False,param=param,allsubj_param=allsubj_param,\
                                                    SUBJECT = [subject], ReshapeAsGrid=1, rearrange_elec=0, low_density = (cfg.DATASET.DENSITY == 'LD'), process_ecog = False, \
                                formant_label = args_.formant_supervision, pitch_label = pitch_label, \
                                intensity_label = intensity_label,LOO=args_.LOO,cv_ind = args_.cv_ind,use_denoise=args_.use_denoise)
        else:
            dataset_test_all[subject] = TFRecordsDataset(cfg, logger, rank=local_rank, world_size=world_size, buffer_size_mb=1024, \
                                            channels=cfg.MODEL.CHANNELS,train=False,param=param,allsubj_param=allsubj_param,\
                                SUBJECT = [subject], ReshapeAsGrid=1, rearrange_elec=0, low_density = (cfg.DATASET.DENSITY == 'LD'), process_ecog = False, \
                        formant_label = args_.formant_supervision, pitch_label = pitch_label, intensity_label = intensity_label,use_denoise=args_.use_denoise)
    ...
    for subject in test_subject_info:
        ...
        if cfg.DATASET.PROD:
            ...
            sample_spec_test2_all[subject] = to_db(F.conv2d(sample_spec_amp_test_all[subject].transpose(-2,-1),hann_win,padding=[10,0]).transpose(-2,-1),cfg.MODEL.NOISE_DB,cfg.MODEL.MAX_DB)
    
        else:
            sample_wave_test = sample_dict_test['wave_batch_all'].to(device).float()
            sample_wave_denoise_test = sample_dict_test['wave_denoise_batch_all'].to(device).float()
            if not cfg.DATASET.DENSITY == 'LD':
                sample_voice_test = sample_dict_test['voice_batch_all'].to(device).float()
                sample_unvoice_test = sample_dict_test['unvoice_batch_all'].to(device).float()
                sample_semivoice_test = sample_dict_test['semivoice_batch_all'].to(device).float()
                sample_plosive_test = sample_dict_test['plosive_batch_all'].to(device).float()
                sample_fricative_test = sample_dict_test['fricative_batch_all'].to(device).float()
            if cfg.MODEL.WAVE_BASED:
                sample_spec_test = sample_dict_test['wave_spec_batch_all'].to(device).float()
                sample_spec_amp_test = sample_dict_test['wave_spec_denoise_amp_batch_all'].to(device).float() if x_amp_from_denoise else sample_dict_test['wave_spec_amp_batch_all'].to(device).float()
                sample_spec_denoise_test = sample_dict_test['wave_spec_denoise_batch_all'].to(device).float()
                # sample_spec_test = wave2spec(sample_wave_test,n_fft=cfg.MODEL.N_FFT,noise_db=cfg.MODEL.NOISE_DB,max_db=cfg.MODEL.MAX_DB)
            else: 
            ...
            if cfg.MODEL.ECOG:
                ecog_test = [sample_dict_test['ecog_batch_all'][i].to(device).float() for i in range(len(sample_dict_test['ecog_batch_all']))]
                ecog_raw_test = [sample_dict_test['ecog_raw_batch_all'][i].to(device).float() for i in range(len(sample_dict_test['ecog_raw_batch_all']))]
                mask_prior_test = [sample_dict_test['mask_all'][i].to(device).float() for i in range(len(sample_dict_test['mask_all']))]
                mni_coordinate_test = sample_dict_test['mni_coordinate_all'].to(device).float()
            else:
                ecog_test = None
                ecog_raw_test = None
                mask_prior_test = None
                mni_coordinate_test = None
            sample_spec_mel_test = sample_dict_test['spkr_batch_all'].to(device).float() if cfg.MODEL.DO_MEL_GUIDE else None
            on_stage_test = sample_dict_test['on_stage_batch_all'].to(device).float()
            on_stage_wider_test = sample_dict_test['on_stage_wider_batch_all'].to(device).float()
            # sample = next(make_dataloader(cfg, logger, dataset, 32, local_rank))
            # sample = (sample / 127.5 - 1.)
    ...
    for epoch in range(cfg.TRAIN.TRAIN_EPOCHS):
        ...
        for sample_dict_train in tqdm(iter(dataset.iterator)):
            ...
            if cfg.DATASET.PROD:
                ...
                wave_orig = reshape_multi_batch(wave_orig, batchsize = batch_size, patient_len = patient_len)
                if not cfg.DATASET.DENSITY == 'LD':
                    sample_voice = sample_dict_train['voice_re_batch_all'].to(device).float()
                    sample_unvoice = sample_dict_train['unvoice_re_batch_all'].to(device).float()
                    sample_semivoice = sample_dict_train['semivoice_re_batch_all'].to(device).float()
                    sample_plosive = sample_dict_train['plosive_re_batch_all'].to(device).float()
                    sample_fricative = sample_dict_train['fricative_re_batch_all'].to(device).float()
                else:
                    sample_voice = None
                    sample_unvoice = None
                ...
                if cfg.MODEL.WAVE_BASED:
                    ...
                    x_orig = sample_dict_train['wave_spec_re_batch_all'].to(device).float()
                    x_orig_amp = sample_dict_train['wave_spec_re_denoise_amp_batch_all'].to(device).float() if x_amp_from_denoise else sample_dict_train['wave_spec_re_amp_batch_all'].to(device).float()
                    x_orig_denoise = sample_dict_train['wave_spec_re_denoise_batch_all'].to(device).float()
                else:
                ...
                x_orig = reshape_multi_batch(x_orig, batchsize = batch_size, patient_len = patient_len)
                x_orig_amp = reshape_multi_batch(x_orig_amp, batchsize = batch_size, patient_len = patient_len)
                x_orig_denoise = reshape_multi_batch(x_orig_denoise, batchsize = batch_size, patient_len = patient_len)
                # hann_win = torch.hann_window(21,periodic=False).reshape([1,1,21,1])
                # hann_win = hann_win/hann_win.sum()
                if cfg.MODEL.WAVE_BASED:
                    x_orig2 = to_db(F.conv2d(x_orig_amp.transpose(-2,-1),hann_win,padding=[10,0]).transpose(-2,-1),cfg.MODEL.NOISE_DB,cfg.MODEL.MAX_DB)
                # x_orig2 = x_orig
                if args_.formant_supervision:
                    formant_label = sample_dict_train['formant_re_batch_all'].to(device).float()
                    formant_label = reshape_multi_batch(formant_label, batchsize = batch_size, patient_len = patient_len)
                else:
                    formant_label = None
                if args_.pitch_supervision:
                    pitch_label = sample_dict_train['pitch_re_batch_all'].to(device).float()
                    pitch_label = reshape_multi_batch(pitch_label, batchsize = batch_size, patient_len = patient_len)
                else:
                    pitch_label = None
                if args_.intensity_supervision:
                    intensity_label = sample_dict_train['intensity_re_batch_all'].to(device).float()
                    intensity_label = reshape_multi_batch(intensity_label, batchsize = batch_size, patient_len = patient_len)
                else:
                    intensity_label = None

                on_stage = sample_dict_train['on_stage_re_batch_all'].to(device).float()
                on_stage_wider = sample_dict_train['on_stage_wider_re_batch_all'].to(device).float()
                ...
                on_stage = reshape_multi_batch(on_stage, batchsize = batch_size, patient_len = patient_len)
                on_stage_wider = reshape_multi_batch(on_stage_wider, batchsize = batch_size, patient_len = patient_len)
                #labels = reshape_multi_batch(labels, batchsize = batch_size, patient_len = patient_len)
                #print (labels)
                labels =list(itertools.chain(labels))
                #print (labels)
                gender_train = reshape_multi_batch(gender_train, batchsize = batch_size, patient_len = patient_len)
                
                
                if cfg.MODEL.ECOG:
                    ecog = [sample_dict_train['ecog_re_batch_all'][j].to(device).float() for j in range(len(sample_dict_train['ecog_re_batch_all']))]
                    ...
                    mni_coordinate = None
                x = x_orig
                x_mel = sample_dict_train['spkr_re_batch_all'].to(device).float() if cfg.MODEL.DO_MEL_GUIDE else None
                if x_mel is not None:
                    x_mel = reshape_multi_batch(x_mel, batchsize = batch_size, patient_len = patient_len)
            else:
            ...
            if (cfg.MODEL.ECOG):
                optimizer.zero_grad()
                Lrec = model(spec2 = x_orig2, spec = x, ecog = ecog, x_denoise = x_orig_denoise,x_mel = x_mel,  mask_prior=mask_prior, on_stage = on_stage,\
                                            on_stage_wider = on_stage_wider, gender=gender_train,\
                                     voice=sample_voice,unvoice = sample_unvoice,semivoice = sample_semivoice,plosive = sample_plosive,fricative = sample_fricative,\
                                        ae = True, tracker = tracker, encoder_guide=cfg.MODEL.W_SUP,duomask=duomask,mni=mni_coordinate,x_amp=x_orig_amp)
                ...
                optimizer.zero_grad()
                #print ('tracker',tracker,tracker.tracks)
                Lrec,tracker   = model(spec2 = x_orig2, spec = x,  x_denoise = x_orig_denoise,x_mel = x_mel,ecog=ecog, mask_prior=None, on_stage = on_stage,\
                                on_stage_wider = on_stage_wider, ae = True, tracker = tracker, encoder_guide=cfg.MODEL.W_SUP,pitch_aug=False,gender=gender_train,\
                                    duomask=duomask,mni=mni_coordinate,debug = False,x_amp=x_orig_amp,hamonic_bias = False,x_amp_from_denoise=x_amp_from_denoise,\
                                voice=sample_voice,unvoice = sample_unvoice,semivoice = sample_semivoice,plosive = sample_plosive,fricative = sample_fricative,\
                                formant_label = formant_label, pitch_label = pitch_label, intensity_label = intensity_label,epoch_current = epoch,n_iter = n_iter_pass, save_path = cfg.OUTPUT_DIR)#epoch<2)
                #if DEBUG:
                #    if n_iter % 50 == 0:
            ...
            #debug save train
            if n_iter % 1000000 == 1:
                save_sample(cfg,x_orig2,x,ecog,mask_prior,mni_coordinate,encoder,decoder,ecog_encoder if cfg.MODEL.ECOG else None,encoder2,\
                    x_denoise=x_orig_denoise,x_mel = x_mel,decoder_mel=decoder_mel if cfg.MODEL.DO_MEL_GUIDE else None,tracker=tracker,\
                        epoch=epoch,label=labels,mode='train',path=cfg.OUTPUT_DIR,linear=cfg.MODEL.WAVE_BASED,\
                            n_fft=cfg.MODEL.N_FFT,duomask=duomask,x_amp=x_orig_amp, \
                                gender=gender_train,on_stage_wider = on_stage)
        ...
        if local_rank == 0:
            ...
            if epoch % save_inter  == 0:    
                for subject in test_subject_info:
                    #print ('epoch',epoch)
                    model.eval()
                    model(sample_spec_test2_all[subject], sample_spec_test_all[subject], x_denoise = sample_spec_denoise_test_all[subject],x_mel = sample_spec_mel_test_all[subject],ecog=ecog_test if \
                    cfg.MODEL.ECOG else None, mask_prior=mask_prior_test if cfg.MODEL.ECOG else None, on_stage = on_stage_test_all[subject],on_stage_wider =\
                        on_stage_wider_test_all[subject], ae = not cfg.MODEL.ECOG, tracker = tracker_test, encoder_guide=cfg.MODEL.W_SUP,pitch_aug=False,\
                            duomask=duomask,debug = False,x_amp=sample_spec_amp_test_all[subject],hamonic_bias = False,gender=gender_test_all[subject])
                    save_sample(cfg, sample_spec_test2_all[subject], sample_spec_test_all[subject],ecog_test_all[subject],mask_prior_test_all[subject],mni_coordinate_test_all[subject],encoder,decoder,\
                        ecog_encoder if cfg.MODEL.ECOG else None,encoder2, x_denoise = sample_spec_denoise_test_all[subject],x_mel = sample_spec_mel_test_all[subject],\
                    decoder_mel=decoder_mel if cfg.MODEL.DO_MEL_GUIDE else None, epoch=epoch, label = sample_label_test_all[subject], mode='test', tracker = tracker_test,\
                        path=cfg.OUTPUT_DIR, linear=cfg.MODEL.WAVE_BASED, n_fft=cfg.MODEL.N_FFT, duomask=duomask, x_amp=sample_spec_amp_test_all[subject],\
...
if __name__ == "__main__":
    ...
    if args_.trainsubject !='':
        ...
        train_subject_info = args_.subject.split(',')#cfg.DATASET.SUBJECT #already splitted
    if args_.testsubject !='':
        test_subject_info = args_.testsubject.split(',')
    else:
    ...
    config_file = 'configs/ecog_style2_a.yaml' if Gender == 'Female' else 'configs/ecog_style2_a_male.yaml'
