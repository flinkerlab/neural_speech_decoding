...
def _run(rank, world_size, fn, defaults, write_log, no_cuda, args_):
    ...
    cfg.MODEL.ONEDCONFIRST  = True if args_.ONEDCONFIRST==1 else False
    ...
    cfg.MODEL.TRANSFORMER.FASTATTENTYPE = args_.fastattentype
    cfg.MODEL.PHONEMEWEIGHT = args_.phone_weight
    cfg.DATASET.SUBJECT = args_.subject.split(',')#[args_.subject]
    ...
    cfg.MODEL.component_regression = args_.component_regression
    cfg.MODEL.CAUSAL = args_.causal
    cfg.MODEL.ANTICAUSAL = args_.anticausal
    cfg.MODEL.freq_single_formant_loss_weight = args_.freq_single_formant_loss_weight
    cfg.MODEL.amp_minmax = args_.amp_minmax
    cfg.MODEL.mapping_layers = args_.mapping_layers
    cfg.MODEL.region_index = args_.region_index
    cfg.MODEL.multiscale= args_.multiscale
    cfg.MODEL.rdropout= args_.rdropout
    ...
    cfg.MODEL.single_patient_mapping = args_.single_patient_mapping
    if args_.amp_minmax==1:
        cfg.MODEL.amp_formant_loss_weight = 1
    else:
        cfg.MODEL.amp_formant_loss_weight = args_.amp_formant_loss_weight
    cfg.MODEL.amp_energy = args_.amp_energy
    
    cfg.MODEL.f0_midi = args_.f0_midi 
    cfg.MODEL.alpha_db = args_.alpha_db
    cfg.MODEL.network_db = args_.network_db
    cfg.MODEL.consistency_loss = args_.consistency_loss
    cfg.MODEL.delta_time = args_.delta_time
    cfg.MODEL.delta_freq = args_.delta_freq
    cfg.MODEL.cumsum = args_.cumsum
    cfg.MODEL.distill = args_.distill
    
    cfg.MODEL.classic_pe = args_.classic_pe
    cfg.MODEL.temporal_down_before = args_.temporal_down_before
    cfg.MODEL.conv_method = args_.conv_method
    cfg.MODEL.classic_attention = args_.classic_attention
    cfg.MODEL.WAVE_BASED = args_.wavebased
    cfg.TRAIN.BATCH_SIZE = args_.batch_size
    cfg.MODEL.BGNOISE_FROMDATA = args_.bgnoise_fromdata
    cfg.MODEL.NOISE_DB = args_.noise_db
    cfg.MODEL.RETURNFILTERSHAPE = args_.return_filtershape
