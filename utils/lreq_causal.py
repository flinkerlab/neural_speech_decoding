...
class Conv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, output_padding=0, dilation=1,
                 ...
                 implicit_lreq=use_implicit_lreq,initial_weight=None,causal=[0,0],anticausal=False):
        ...
        self.causal = make_tuple(causal, 2) if (isinstance(causal,int) or isinstance(causal,bool)) else causal
    ...
    def forward(self, x):
        szs = x.shape[2:]
        if self.transpose:
            padding = [self.dilation[i]*(self.kernel_size[i]-1) if self.causal[i] else self.padding[i] for i in range(2)]
            if self.transform_kernel:
                padding_ = [int(math.ceil((self.kernel_size[i]+1+self.stride[i]-2)/self.stride[i])) if self.causal[i] else 0 for i in range(2)]
            else:
                padding_ = [int(math.ceil((self.kernel_size[i]+self.stride[i]-2)/self.stride[i]))  if self.causal[i] else 0 for i in range(2)]
            output_padding = [1 if self.causal[i] else self.output_padding[i] for i in range(2)]
            if np.any(np.array(padding_)):
                if self.anticausal:
                    x = F.pad(x, (0,padding_[1], 0,padding_[0]))
                else:
                    x = F.pad(x, (padding_[1], 0, padding_[0], 0))
            ...
            if not self.implicit_lreq:
                ...
                if bias is not None:
                    bias = bias * self.lrmul
                out= F.conv_transpose2d(x, w * self.std, bias, stride=self.stride,
                                          padding=padding, output_padding=output_padding,
                                          dilation=self.dilation, groups=self.groups)
            else:
                out= F.conv_transpose2d(x, w, self.bias, stride=self.stride, padding=padding,
                                          output_padding=output_padding, dilation=self.dilation,
                                          groups=self.groups)
            if any(self.causal):
                if self.anticausal:
                    return out[:,:,:self.stride[0]*szs[0],:self.stride[1]*szs[1]]
                else:
                    return out[:,:,-self.stride[0]*szs[0]:,-self.stride[1]*szs[1]:]
            else:
                return out
        else:
            padding = [0 if self.causal[i] else self.padding[i] for i in range(2)]
            padding_ = [self.dilation[i]*(self.kernel_size[i]-1) if self.causal[i] else 0 for i in range(2)]
            if np.any(np.array(padding_)):
                if self.anticausal:
                    x = F.pad(x, (0,padding_[1], 0,padding_[0]))
                else:
            ...
            if not self.implicit_lreq:
                ...
                if bias is not None:
                    bias = bias * self.lrmul
                return F.conv2d(x, w * self.std, bias, stride=self.stride, padding=padding,
                                dilation=self.dilation, groups=self.groups)
            else:
                return F.conv2d(x, w, self.bias, stride=self.stride, padding=padding,
                                dilation=self.dilation, groups=self.groups)

class Conv3d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, output_padding=0, dilation=1,
                 groups=1, bias=True, gain=np.sqrt(2.0), transpose=False, transform_kernel=False, lrmul=1.0,
                 implicit_lreq=use_implicit_lreq,initial_weight=None,causal=[0,0,0],anticausal=False):
        ...
        self.transform_kernel = transform_kernel
        self.initial_weight = initial_weight
        self.causal = make_tuple(causal, 3) if (isinstance(causal,int) or isinstance(causal,bool)) else causal
        self.anticausal = anticausal
        if transpose:
            self.weight = Parameter(torch.Tensor(in_channels, out_channels // groups, *self.kernel_size))
        else:
    ...
    def forward(self, x):
        szs = x.shape[2:]
        if self.transpose:
            padding = [self.dilation[i]*(self.kernel_size[i]-1) if self.causal[i] else self.padding[i] for i in range(3)]
            if self.transform_kernel:
                padding_ = [int(math.ceil((self.kernel_size[i]+1+self.stride[i]-2)/self.stride[i])) if self.causal[i] else 0 for i in range(3)]
            else:
                padding_ = [int(math.ceil((self.kernel_size[i]+self.stride[i]-2)/self.stride[i]))  if self.causal[i] else 0 for i in range(3)]
            output_padding = [1 if self.causal[i] else self.output_padding[i] for i in range(3)]
            if np.any(np.array(padding_)):
                if self.anticausal:
                    x = F.pad(x, (0,padding_[2], 0,padding_[1], 0, padding_[0]))
                else:
                    x = F.pad(x, (padding_[2], 0, padding_[1], 0, padding_[0], 0))
            ...
            if not self.implicit_lreq:
                ...
                if bias is not None:
                    bias = bias * self.lrmul
                out = F.conv_transpose3d(x, w * self.std, bias, stride=self.stride,
                                          padding=padding, output_padding=output_padding,
                                          dilation=self.dilation, groups=self.groups)
            else:
                out= F.conv_transpose3d(x, w, self.bias, stride=self.stride, padding=padding,
                                          output_padding=output_padding, dilation=self.dilation,
                                          groups=self.groups)
            if any(self.causal):
                if self.anticausal:
                    return out[:,:,:self.stride[0]*szs[0],:self.stride[1]*szs[1],:self.stride[2]*szs[2]]
                else:
                    return out[:,:,-self.stride[0]*szs[0]:,-self.stride[1]*szs[1]:,-self.stride[2]*szs[2]:]
            else:
                return out
        else:
            padding = [0 if self.causal[i] else self.padding[i] for i in range(3)]
            padding_ = [self.dilation[i]*(self.kernel_size[i]-1) if self.causal[i] else 0 for i in range(3)]
            if np.any(np.array(padding_)):
                if self.anticausal:
                    x = F.pad(x, (0,padding_[2], 0,padding_[1], 0, padding_[0]))
                else:
            ...
            if not self.implicit_lreq:
                ...
                if bias is not None:
                    bias = bias * self.lrmul
                return F.conv3d(x, w * self.std, bias, stride=self.stride, padding=padding,
                                dilation=self.dilation, groups=self.groups)
            else:
                return F.conv3d(x, w, self.bias, stride=self.stride, padding=padding,
                                dilation=self.dilation, groups=self.groups)
...
class Conv1d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, output_padding=0, dilation=1,
                 groups=1, bias=True, gain=np.sqrt(2.0), transpose=False, transform_kernel=False, lrmul=1.0,
                 implicit_lreq=use_implicit_lreq, bias_initial = 0.,causal=False,anticausal=False,padmode = 'constant'):
        ...
        self.causal = make_tuple(causal, 1) if (isinstance(causal,int) or isinstance(causal,bool)) else causal
        self.anticausal = anticausal
        self.groups = groups
        self.gain = gain
        self.lrmul = lrmul
    ...
    def forward(self, x):
        szs = x.shape[2:]
        if self.transpose:
            padding = [self.dilation[i]*(self.kernel_size[i]-1) if self.causal[i] else self.padding[i] for i in range(1)]
            if self.transform_kernel:
                padding_ = [int(math.ceil((self.kernel_size[i]+1+self.stride[i]-2)/self.stride[i])) if self.causal[i] else 0 for i in range(1)]
            else:
                padding_ = [int(math.ceil((self.kernel_size[i]+self.stride[i]-2)/self.stride[i])) if self.causal[i] else 0 for i in range(1)]
            if self.transform_kernel:
                output_padding = [0 if self.causal[i] else self.output_padding[i] for i in range(1)]
            else:
                output_padding = [1 if self.causal[i] else self.output_padding[i] for i in range(1)]
            if np.any(np.array(padding_)):
                if self.anticausal:
            ...
            if not self.implicit_lreq:
                ...
                if bias is not None:
                    bias = bias * self.lrmul
                out = F.conv_transpose1d(x, w * self.std, bias, stride=self.stride,
                                        padding=padding, output_padding=output_padding,
                                        dilation=self.dilation, groups=self.groups)
            else:
                out = F.conv_transpose1d(x, w, self.bias, stride=self.stride, padding=padding,
                                        output_padding=output_padding, dilation=self.dilation,
                                        groups=self.groups)
            if any(self.causal):
                if self.anticausal:
            ...
            padding_ = [self.dilation[i]*(self.kernel_size[i]-1) if self.causal[i] else 0 for i in range(1)]
            if np.any(np.array(padding_)):
                if self.anticausal:
                    x = F.pad(x, (0,padding_[0]),mode = self.padmode)
                else:
            ...
            if not self.implicit_lreq:
                ...
                if bias is not None:
                    bias = bias * self.lrmul
                return F.conv1d(x, w * self.std, bias, stride=self.stride, padding=padding,
                                dilation=self.dilation, groups=self.groups)
            else:
                return F.conv1d(x, w, self.bias, stride=self.stride, padding=padding,
                                dilation=self.dilation, groups=self.groups)

class ConvTranspose2d(Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, output_padding=0, dilation=1,
                 groups=1, bias=True, gain=np.sqrt(2.0), transform_kernel=False, lrmul=1.0, implicit_lreq=use_implicit_lreq,causal=[0,0],anticausal=False):
...
class ConvTranspose1d(Conv1d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, output_padding=0, dilation=1,
                 groups=1, bias=True, gain=np.sqrt(2.0), transform_kernel=False, lrmul=1.0, implicit_lreq=use_implicit_lreq,causal=False,anticausal=False):
