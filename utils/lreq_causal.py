...
class Conv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.transpose:
            ...
            output_padding = [
                1 if self.causal[i] else self.output_padding[i] for i in range(2)
            ]
            ...
            if any(self.causal):
                if self.anticausal:
                    return out[
                        :, :, : self.stride[0] * szs[0], : self.stride[1] * szs[1]
                    ]
                else:
                    return out[
                        :, :, -self.stride[0] * szs[0] :, -self.stride[1] * szs[1] :
...
class Conv3d(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.transpose:
            ...
            output_padding = [
                1 if self.causal[i] else self.output_padding[i] for i in range(3)
            ]
            if np.any(np.array(padding_)):
                if self.anticausal:
