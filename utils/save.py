...
#from net_formant_ran0322 import mel_scale, hz2ind, amplitude
from net_formant_ran import mel_scale, hz2ind, amplitude
...
def mygriffinlim(msgram, hop_length) :
   wave_gt = np.ndarray([msgram.shape[0],griffinlim(msgram[0]**0.5,hop_length=hop_length).shape[0]])
...
def voicing_coloring(sample,amplitude):
   for b in range(sample.shape[0]):
      for t in range(sample.shape[2]):
         sample[b,:,t]=sample[b,0:1,t]*(amplitude[b,0,t]*torch.tensor([0.,0.35,0.67]).to(device) + amplitude[b,1,t]*torch.tensor([1.0,0.87,0.]).to(device)).unsqueeze(dim=-1) # voicing for blue, unvoicing for yellow
   return sample
   ...
   if formant_line:
      ...
      if ecog is not None:
         ...
         if which_formant == 'noise':
            minimum = hz2ind(ecog['freq_formants_noise_hz'][:,-1] - ecog['bandwidth_formants_noise_hz'][:,-1]/2,n_fft).clamp(min=0).squeeze().detach().cpu().numpy()
            maximum = hz2ind(ecog['freq_formants_noise_hz'][:,-1] + ecog['bandwidth_formants_noise_hz'][:,-1]/2,n_fft).clamp(min=0).squeeze().detach().cpu().numpy()
            ax.fill_between(range(minimum.shape[0]),minimum,maximum,where=(minimum<=maximum),color='tab:gray',alpha=0.2) 
      #ax.legend()
   else:
      if title=='loudness':
         ...
         ax.plot(200*alpha[:,0].squeeze().detach().cpu().numpy(),linewidth=2,color=clrs[0])
         if ecog is not None:
            # for i in range(alpha.shape[1]): 
            ax.plot(200*alpha_ecog[:,0].squeeze().detach().cpu().numpy(),linewidth=2,color=clrs[0],linestyle='--')
...
def save_sample(cfg,sample2,sample,ecog,mask_prior,mni,encoder,decoder,ecog_encoder,encoder2,epoch,label,x_denoise=None,decoder_mel=None,x_mel=None,mode='test',path='training_artifacts/formantsysth_voicingandunvoicing_loudness',tracker=None,linear=False,n_fft=256,duomask=True,x_amp=None,gender='Female',sample_wave=None,sample_wave_denoise=None,on_stage_wider = None,auto_regressive=False,seq_out_start=None,suffix=''):
   ...
   with torch.no_grad():
      ...
      if decoder_mel is None:
         fig,axs = plt.subplots(9,sample.shape[0],figsize=(5*sample.shape[0],23*(4 if linear else 1))) if (ecog_encoder is None and encoder2 is None) else plt.subplots(11,sample.shape[0],figsize=(5*sample.shape[0]//2,28*4//2))
      else:
         fig,axs = plt.subplots(10,sample.shape[0],figsize=(5*sample.shape[0],27*(4 if linear else 1))) if (ecog_encoder is None and encoder2 is None) else plt.subplots(11,sample.shape[0],figsize=(5*sample.shape[0]//2,28*4//2))
      ...
      for i in range(0,sample.shape[0],1):
         ...
         components = encoder(sample_in,duomask=duomask,x_denoise=sample_in_denoise,noise_level = None,x_amp=x_amp_in,gender=gender_in,onstage = on_stage_wider[i:np.minimum(i+1,sample.shape[0])])
         rec = decoder(components,onstage=on_stage_in)
         # import pdb; pdb.set_trace()
         decoder.return_wave = True
         rec_denoise,rec_denoise_wave = decoder(components,enable_bgnoise=False,onstage=on_stage_in)
         ...
         sample_in_all = torch.cat([sample_in_all.to(device),sample_in_.to(device)],dim=0)
         ...
         rec_denoise_all = torch.cat([rec_denoise_all.to(device),rec_denoise.to(device)],dim=0)
         #rec_denoise_wave_all = torch.cat([rec_denoise_wave_all.to(device),rec_denoise_wave.to(device)],dim=0)
         components_all += [components]
         if ecog_encoder is not None or encoder2 is not None:
            if cfg.VISUAL.A2A:
               components_ecog = encoder2(sample_in2,duomask=duomask,x_denoise=sample_in_denoise,noise_level = None,x_amp=x_amp_in,gender=gender_in,onstage = on_stage_wider[i:np.minimum(i+1,sample.shape[0])])
            else:
               if auto_regressive:
                  components_ecog = ecog_encoder.generate(ecog_in,mask_prior_in,mni=mni_in,seq_out_start=seq_out_start,seq_len=128)#,gender=gender_in)
               else:
                  if sample.shape[0] >50: #for perception multi, mapping layer case, select sample's specific layer
                     print ('percept multi mapping, {} th sample, {} th layer'.format(i,i//50),)
                     components_ecog = ecog_encoder(ecog_in,mask_prior_in,mni=mni_in,single_patient_mapping=i//50)
                  else:
                     components_ecog = ecog_encoder(ecog_in,mask_prior_in,mni=mni_in)#,gender=gender_in)
            rec_ecog = decoder(components_ecog,onstage=on_stage_wider)
            rec_ecog = rec_ecog.repeat(1,3,1,1)
            rec_ecog = rec_ecog * 0.5 + 0.5
            decoder.return_wave = True
            rec_ecog_denoise,rec_ecog_denoise_wave = decoder(components_ecog,enable_bgnoise=False,onstage=on_stage_wider)
            decoder.return_wave = False
            rec_ecog_denoise = rec_ecog_denoise.repeat(1,3,1,1)
            rec_ecog_denoise = rec_ecog_denoise * 0.5 + 0.5
            rec_ecog_all = torch.cat([rec_ecog_all.to(device),rec_ecog.to(device)],dim=0)
            rec_denoise_ecog_all = torch.cat([rec_denoise_ecog_all.to(device),rec_ecog_denoise.to(device)],dim=0)
            rec_denoise_ecog_wave_all = torch.cat([rec_denoise_ecog_wave_all.to(device),rec_ecog_denoise_wave.to(device)],dim=0)
      ...
      for i in range(0,sample.shape[0],1):
         ...
         subfigure_plot(axs[0,i],sample_in_all[i:i+1],components_all[i],n_mels,formant_line=False,title=str(i)+':'+'\''+labels[i]+'\'',linear=linear,n_fft=n_fft)
         subfigure_plot(axs[1,i],sample_in_all[i:i+1],components_all[i],n_mels,formant_line=False,title='loudness',linear=linear,n_fft=n_fft,power=decoder.power_synth,ecog = None if (ecog_encoder is None and encoder2 is None) else components_ecog_all[i])
         subfigure_plot(axs[2,i],sample_in_all[i:i+1],components_all[i],n_mels,formant_line=True,title='formants_hamon',linear=linear,n_fft=n_fft,ecog = None if (ecog_encoder is None and encoder2 is None) else components_ecog_all[i])
         subfigure_plot(axs[3,i],sample_in_all[i:i+1],components_all[i],n_mels,which_formant='noise',formant_line=True,title='formants_noise',linear=linear,n_fft=n_fft,ecog = None if (ecog_encoder is None and encoder2 is None) else components_ecog_all[i])
         subfigure_plot(axs[4,i],sample_in_all[i:i+1],components_all[i],n_mels,formant_line=False,title='amplitude_hamon',linear=linear,n_fft=n_fft,ecog = None if (ecog_encoder is None and encoder2 is None) else components_ecog_all[i])
         subfigure_plot(axs[5,i],sample_in_all[i:i+1],components_all[i],n_mels,formant_line=False,title='amplitude_noise',linear=linear,n_fft=n_fft,ecog = None if (ecog_encoder is None and encoder2 is None) else components_ecog_all[i])
         subfigure_plot(axs[6,i],sample_in_color_voicing,components_all[i],n_mels,formant_line=False,title='alpha',linear=linear,n_fft=n_fft,ecog = None if (ecog_encoder is None and encoder2 is None) else components_ecog_all[i])
      ...
      if linear:
         if use_GL:
            rec_wave = mygriffinlim(amplitude(2*rec_all[:,0].to(device)-1,noise_db,max_db,trim_noise=True).transpose(-2,-1).detach().cpu().numpy(),129)
            rec_all = amplitude(torch.cat((2*rec_all[:,0]-1).unbind(),0).transpose(-2,-1).detach().cpu().numpy(),noise_db,max_db,trim_noise=True)
         else:
            rec_all = amplitude(torch.cat((2*rec_all[:,0]-1).unbind(),0).transpose(-2,-1).detach().cpu().numpy(),noise_db,max_db,trim_noise=True)
            rec_wave = spsi(rec_all,(n_fft-1)*2,128)
         scipy.io.wavfile.write(f2+'.wav',16000,rec_wave)
         if use_GL:
            rec_wave_denoise = mygriffinlim(amplitude(2*rec_denoise_all[:,0].to(device)-1,noise_db,max_db,trim_noise=True).transpose(-2,-1).detach().cpu().numpy(),129)
            rec_denoise_all = amplitude(torch.cat((2*rec_denoise_all[:,0].to(device)-1).unbind(),0).transpose(-2,-1).detach().cpu().numpy(),noise_db,max_db,trim_noise=True)
         else:
            rec_denoise_all = amplitude(torch.cat((2*rec_denoise_all[:,0].to(device)-1).unbind(),0).transpose(-2,-1).detach().cpu().numpy(),noise_db,max_db,trim_noise=True)
         ...
         if ecog_encoder is not None or encoder2 is not None:
            if use_GL:
               rec_wave_ecog = mygriffinlim(amplitude(2*rec_ecog_all[:,0].to(device)-1,noise_db,max_db,trim_noise=True).transpose(-2,-1).detach().cpu().numpy(),129)
               rec_ecog_all = amplitude(torch.cat((2*rec_ecog_all[:,0].to(device)-1).unbind(),0).transpose(-2,-1).detach().cpu().numpy(),noise_db,max_db,trim_noise=True)
            else:
               rec_ecog_all = amplitude(torch.cat((2*rec_ecog_all[:,0].to(device)-1).unbind(),0).transpose(-2,-1).detach().cpu().numpy(),noise_db,max_db,trim_noise=True)
               rec_wave_ecog = spsi(rec_ecog_all,(n_fft-1)*2,128)
            scipy.io.wavfile.write(f2+'ecog.wav',16000,rec_wave_ecog)
            if use_GL:
               rec_wave_denoise_ecog = mygriffinlim(amplitude(2*rec_denoise_ecog_all[:,0].to(device)-1,noise_db,max_db,trim_noise=True).transpose(-2,-1).detach().cpu().numpy(),129)
               rec_denoise_ecog_all = amplitude(torch.cat((2*rec_denoise_ecog_all[:,0].to(device)-1).to(device).unbind(),0).transpose(-2,-1).detach().cpu().numpy(),noise_db,max_db,trim_noise=True)
            else:
               rec_denoise_ecog_all = amplitude(torch.cat((2*rec_denoise_ecog_all[:,0].to(device)-1).to(device).unbind(),0).transpose(-2,-1).detach().cpu().numpy(),noise_db,max_db,trim_noise=True)
      ...
      if mode=='test':
         ...
         for i,j in components_all_np.items():
            components_all_np[i] = torch.cat(components_all_np[i],0).detach().cpu().numpy()
         ...
         if ecog_encoder is not None or encoder2 is not None:
            ...
            for i,j in components_ecog_all_np.items():
               components_ecog_all_np[i] = torch.cat(components_ecog_all_np[i],0).detach().cpu().numpy()
            # components_ecog_all_np = {i:j.clone() for i,j in components_ecog_all.items().detach().cpu().numpy()}
         save_dict = {"components":components_all_np,
                     "on_stage": on_stage_wider.detach().cpu().numpy(),
                     "org":amplitude(torch.cat((2*sample_in_all[:,0]-1).unbind(),0).transpose(-2,-1).detach().cpu().numpy(),noise_db,max_db,trim_noise=True),
                     "rec":rec_all,
                     "org_":torch.cat((2*sample_in_all[:,0]-1).unbind(),0).transpose(-2,-1).detach().cpu().numpy(),
                     "org_denoise":amplitude(torch.cat(x_denoise.unbind(),0).transpose(-2,-1).detach().cpu().numpy(),noise_db,max_db,trim_noise=True),
